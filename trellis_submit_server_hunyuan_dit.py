#!/usr/bin/env python3
"""
Subnet 17 (404-GEN) - HunyuanDiT + TRELLIS Generation Server
Purpose: HTTP server for high-quality 3D model generation using HunyuanDiT and TRELLIS
...
# Generate 3D model
curl -X POST "http://localhost:8098/generate/" \
  -F "prompt=a blue ceramic vase with red trim" \
  -F "seed=42"

# Get asset information
curl "http://localhost:8098/assets/"

# Download compressed PLY file
curl "http://localhost:8098/assets/gaussian_splatting_ply" -o model.ply.spz
"""
...
# Configuration
GENERATION_CONFIG = {
    'output_dir': './trellis_hunyuan_outputs',
...
class HunyuanTrellisGenerator:
    def __init__(self):
        ...
        print("🔧 HunyuanDiT + TRELLIS Generator initialized")
        self.ready = True
    ...
    def generate_3d_model(self, prompt: str, seed: int = 42) -> Optional[Tuple[bytes, Optional[bytes]]]:
        ...
        with self.generation_lock:
            ...
            try:
                print(f"🎯 Starting HunyuanDiT + TRELLIS generation for: '{prompt}' (seed: {seed})")
                ...
                # enhanced_prompt = f"professional 3D render, Create 3D game asset, isometric view version, {prompt}, highly detailed, photorealistic, studio lighting, clean white background, isometric view, high quality materials, precise geometry, sharp details, professional photography, 8k resolution, centered composition"
                # enhanced_prompt = f" {prompt}, professional 3D render, Create 3D game asset, isometric view version, highly detailed, photorealistic, studio lighting, isometric view, high quality materials, precise geometry, sharp details, professional photography, 8k resolution, centered composition, clean white background" 
                # enhanced_prompt = f" {prompt}, professional 3D render, Create 3D game asset, isometric view version, highly detailed, photorealistic, studio lighting, isometric view, high quality materials, precise geometry, sharp details, professional photography, 8k resolution, clean white background" 
                # enhanced_prompt = f"professional 3D render, Create 3D game asset, isometric view version, Create 3D game asset, isometric view version, {prompt}, highly detailed, photorealistic, studio lighting, clean white background, isometric view, high quality materials, precise geometry, sharp details, professional photography, 8k resolution, centered composition"               
                ...
                # Keep HunyuanDiT pipeline loaded for faster subsequent generations
                # self._unload_hunyuan_pipeline()
                ...
                if GENERATION_CONFIG.get('enable_object_centering', True):
                    ...
                    try:
                        ...
                        generation_asset.add_asset(AssetType.HUNYUAN_IMAGE, centered_image)
                    except Exception as e:
                        print(f"⚠️ Object centering failed: {e}")
                        print("   Continuing with original image...")
                ...
                # Keep TRELLIS pipeline loaded for faster subsequent generations
                ...
                print(f"🎉 HunyuanDiT + TRELLIS generation completed in {generation_time:.2f}s")
                ...
                print(f"❌ HunyuanDiT + TRELLIS generation failed: {e}")
                traceback.print_exc()
                
                # Cleanup on failure - only unload if we're out of memory
                # self._unload_hunyuan_pipeline()
                # self._unload_trellis_pipeline()
...
app = FastAPI(title="HunyuanDiT + TRELLIS Generation Server", version="1.0.0")

# Initialize global generator
generator = HunyuanTrellisGenerator()
...
async def generate_3d_model_endpoint(
    ...
    if return_compressed:
        try:
            ...
            return Response(
                ...
                headers={
                    "Content-Disposition": f"attachment; filename=hunyuan_trellis_model_{seed}.ply.spz",
                    "X-Generation-Seed": str(seed),
                    "X-Generation-Prompt": prompt,
                    "X-Model-Format": "gaussian_splatting_ply",
                    "X-Pipeline": "hunyuan_trellis",
                    "X-Compression": "spz",
    ...
    return Response(
        ...
        media_type="application/octet-stream",
        headers={
            "Content-Disposition": f"attachment; filename=hunyuan_trellis_model_{seed}.ply",
            "X-Generation-Seed": str(seed),
            "X-Generation-Prompt": prompt,
            "X-Model-Format": "gaussian_splatting_ply",
            "X-Pipeline": "hunyuan_trellis",
...
async def generate_image_endpoint(
    ...
    """Generate image only using HunyuanDiT (without 3D generation)."""
    ...
    try:
        ...
        # prompt = f"professional 3D render, Create 3D game asset, isometric view version, {prompt}, highly detailed, photorealistic, studio lighting, clean white background, isometric view, high quality materials, precise geometry, sharp details, professional photography, 8k resolution, centered composition"
        # prompt = f" {prompt}, professional 3D render, Create 3D game asset, isometric view version, highly detailed, photorealistic, studio lighting, high quality materials, precise geometry, sharp details, professional photography, 8k resolution, centered composition, clean white background"                
        # prompt = f"professional 3D render, Create 3D game asset, isometric view version, Create 3D game asset, isometric view version, {prompt}, highly detailed, photorealistic, studio lighting, clean white background, isometric view, high quality materials, precise geometry, sharp details, professional photography, 8k resolution, centered composition"
        # prompt = prompt
        with torch.no_grad():
            image = generator.hunyuan_pipeline(
                prompt=prompt,
                seed=seed
...
async def get_asset_file(asset_type: str):
    ...
    try:
        ...
        filename = f"hunyuan_trellis_{asset_type}"
        
        if asset_type_enum == AssetType.HUNYUAN_IMAGE:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="HunyuanDiT + TRELLIS Generation Server")
    ...
    print(f"Starting HunyuanDiT + TRELLIS Generation Server on {args.host}:{args.port}")
    print("=" * 80)
