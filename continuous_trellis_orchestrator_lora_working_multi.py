...
- DUAL-PORT PARALLEL generation with prompt optimization
"""
...
try:
    ...
    print("⚠️ aiohttp not available - will use requests fallback for parallel generation")
...
class ContinuousTrellisOrchestrator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Initialize gold prompts count in statistics if reproducibility system is available
    ...
    def is_validator_available(self, validator: ValidatorState) -> bool:
        ...
        if validator.cooldown_until and current_time < validator.cooldown_until:
            ...
            self.logger.debug(f"⏳ Validator UID {validator.uid} not available: COOLDOWN ({cooldown_remaining:.1f}s remaining)")
            return False
            ...
            if self.config.get('enable_prompt_optimization', True):
                ...
                if (REPRODUCIBILITY_SYSTEM_AVAILABLE and 
                    ...
                    self.config.get('enable_reproducibility_optimization', True)):
                        ...
                        # Log gold prompts availability
                        gold_prompts_count = len(self.reproducibility_system.gold_standard_results)
                        self.logger.debug(f"📚 Using {gold_prompts_count} gold prompts for reproducibility optimization")
    ...
    async def generate_3d_model_dual_port(self, task: TaskRecord) -> Optional[Dict[str, Any]]:
        """Generate 3D model using dual-port approach: port1 for optimized, port2 for original"""
        self.logger.info(f"🎨 Generating 3D model with dual-port approach: '{task.prompt}' (task: {task.task_id})")
        ...
        try:
            ...
            # Log the dual-port approach
            self.logger.info(f"🚀 DUAL-PORT GENERATION APPROACH:")
            self.logger.info(f"   Port {self.config['port1']}: Optimized prompt '{cleaned_prompt[:50]}...'")
            self.logger.info(f"   Port {self.config['port2']}: Original prompt '{task.prompt[:50]}...'")
            self.logger.info(f"   LoRA: {lora_info['lora_name']} via {endpoint}")
            
            # Clear cache on both servers
            self.priority_coordinator.clear_server_cache()
            ...
            # Step 3: Generate on both ports simultaneously using async
            port1_url = f"http://localhost:{self.config['port1']}{endpoint}"
            port2_url = f"http://localhost:{self.config['port2']}{endpoint}"
            
            # Create async generation tasks for both ports
            async def generate_on_port(port_num, url, prompt, seed):
                """Generate on a specific port asynchronously"""
                self.logger.info(f"🎨 Starting generation on port {port_num} with prompt: '{prompt[:50]}...'")
                ...
                try:
                    ...
                    if AIOHTTP_AVAILABLE:
                        async with aiohttp.ClientSession() as session:
                            async with session.post(
                                ...
                                if response.status == 200:
                                    ...
                                    self.logger.info(f"✅ Port {port_num} generation completed: {len(content):,} bytes")
                                    return {'success': True, 'data': content, 'port': port_num}
                                else:
                                    self.logger.error(f"❌ Port {port_num} generation failed: HTTP {response.status}")
                                    return {'success': False, 'error': f'HTTP {response.status}', 'port': port_num}
                    else:
                        # Fallback to requests if aiohttp not available
                        self.logger.warning(f"⚠️ aiohttp not available, using requests fallback for port {port_num}")
                        import requests
                        response = requests.post(
                            url,
                            data={
                        ...
                        if response.status_code == 200:
                            ...
                            self.logger.info(f"✅ Port {port_num} generation completed (requests): {len(content):,} bytes")
                            return {'success': True, 'data': content, 'port': port_num}
                        else:
                            self.logger.error(f"❌ Port {port_num} generation failed: HTTP {response.status_code}")
                            return {'success': False, 'error': f'HTTP {response.status_code}', 'port': port_num}
                except Exception as e:
                    self.logger.error(f"❌ Port {port_num} generation exception: {e}")
                    return {'success': False, 'error': str(e), 'port': port_num}
            
            # Start both generations simultaneously
            self.logger.info(f"🚀 Starting PARALLEL generation on both ports...")
            self.logger.info(f"   Port {self.config['port1']}: '{cleaned_prompt[:50]}...'")
            self.logger.info(f"   Port {self.config['port2']}: '{task.prompt[:50]}...'")
            
            # Create tasks for both ports
            port1_task = generate_on_port(self.config['port1'], port1_url, cleaned_prompt, deterministic_seed)
            port2_task = generate_on_port(self.config['port2'], port2_url, task.prompt, deterministic_seed)
            
            # Wait for both to complete (they run in parallel)
            port1_result, port2_result = await asyncio.gather(port1_task, port2_task, return_exceptions=True)
            
            # Handle any exceptions from gather
            if isinstance(port1_result, Exception):
                self.logger.error(f"❌ Port {self.config['port1']} generation failed with exception: {port1_result}")
                port1_result = {'success': False, 'error': str(port1_result), 'port': self.config['port1']}
            
            if isinstance(port2_result, Exception):
                self.logger.error(f"❌ Port {self.config['port2']} generation failed with exception: {port2_result}")
                port2_result = {'success': False, 'error': str(port2_result), 'port': self.config['port2']}
            
            # Check if both generations succeeded
            if port1_result['success'] and port2_result['success']:
                port1_ply_data = port1_result['data']
                port2_ply_data = port2_result['data']
                
                self.logger.info(f"✅ PARALLEL dual-port generation successful!")
                self.logger.info(f"   Port {self.config['port1']}: {len(port1_ply_data):,} bytes")
                self.logger.info(f"   Port {self.config['port2']}: {len(port2_ply_data):,} bytes")
                
                # Save both PLY files if enabled
                if self.config['save_intermediate_results']:
                    timestamp = int(time.time())
                    port1_file = self.output_dir / f"task_{task.task_id}_port1_{timestamp}.ply.spz"
                    port2_file = self.output_dir / f"task_{task.task_id}_port2_{timestamp}.ply.spz"
                    
                    with open(port1_file, 'wb') as f:
                        f.write(port1_ply_data)
                    with open(port2_file, 'wb') as f:
                        f.write(port2_ply_data)
                    
                    task.compressed_file_path = str(port1_file)  # Use port1 as primary for now
                ...
                return {
                    'port1_ply_data': port1_ply_data,
                    'port2_ply_data': port2_ply_data,
                    'port1_url': port1_url,
                    'port2_url': port2_url,
                ...
                self.logger.error(f"❌ PARALLEL dual-port generation failed:")
                if not port1_result['success']:
                    self.logger.error(f"   Port {self.config['port1']}: {port1_result.get('error', 'Unknown error')}")
                if not port2_result['success']:
                    self.logger.error(f"   Port {self.config['port2']}: {port2_result.get('error', 'Unknown error')}")
            ...
            if port1_response.status_code == 200 and port2_response.status_code == 200:
                port1_ply_data = port1_response.content
                port2_ply_data = port2_response.content
                
                self.logger.info(f"✅ Dual-port generation successful in {generation_time:.2f}s")
                self.logger.info(f"   Port {self.config['port1']}: {len(port1_ply_data):,} bytes")
                self.logger.info(f"   Port {self.config['port2']}: {len(port2_ply_data):,} bytes")
                ...
                self.logger.error(f"❌ Dual-port generation failed:")
                if port1_response.status_code != 200:
                    self.logger.error(f"   Port {self.config['port1']}: HTTP {port1_response.status_code}")
                if port2_response.status_code != 200:
                    self.logger.error(f"   Port {self.config['port2']}: HTTP {port2_response.status_code}")
                
                self.priority_coordinator.mark_priority_job_end(task.task_id)
                return None
        
        except Exception as e:
            self.logger.error(f"❌ Dual-port generation exception: {e}")
            self.priority_coordinator.mark_priority_job_end(task.task_id)
            return None
    
    async def validate_model_dual_port(self, task: TaskRecord, dual_port_result: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Validate both PLY files using subnet validator and compare scores"""
        if not self.config['validate_generations']:
            return None
        
        self.logger.info(f"📊 Validating dual-port models: '{task.prompt[:50]}...'")
        
        try:
            validation_start = time.time()
            
            # Extract PLY data from both ports
            port1_ply_data = dual_port_result['port1_ply_data']
            port2_ply_data = dual_port_result['port2_ply_data']
            optimized_prompt = dual_port_result['optimized_prompt']
            original_prompt = dual_port_result['original_prompt']
            
            self.logger.info(f"🔍 Validating port {self.config['port1']} (optimized prompt)...")
            port1_score = await self._validate_with_subnet_validator(
                original_prompt, optimized_prompt, port1_ply_data, self.config['port1']
            )
            
            self.logger.info(f"🔍 Validating port {self.config['port2']} (original prompt)...")
            port2_score = await self._validate_with_subnet_validator(
                original_prompt, original_prompt, port2_ply_data, self.config['port2']
            )
            
            validation_time = time.time() - validation_start
            task.validation_time = validation_time
            
            if port1_score is not None and port2_score is not None:
                # Compare scores and choose the better one
                if port1_score > port2_score:
                    better_port = self.config['port1']
                    better_score = port1_score
                    better_ply_data = port1_ply_data
                    better_prompt = optimized_prompt
                    self.logger.info(f"🏆 Port {self.config['port1']} wins with score {port1_score:.4f} vs {port2_score:.4f}")
                else:
                    better_port = self.config['port2']
                    better_score = port2_score
                    better_ply_data = port2_ply_data
                    better_prompt = original_prompt
                    self.logger.info(f"🏆 Port {self.config['port2']} wins with score {port2_score:.4f} vs {port1_score:.4f}")
                
                # Update task with the better score
                task.local_validation_score = better_score
                
                self.logger.info(f"✅ Dual-port validation completed in {validation_time:.2f}s")
                self.logger.info(f"   Port {self.config['port1']}: {port1_score:.4f}")
                self.logger.info(f"   Port {self.config['port2']}: {port2_score:.4f}")
                self.logger.info(f"   Winner: Port {better_port} with score {better_score:.4f}")
                
                self.stats['successful_validations'] += 1
                self.stats['total_validation_time'] += validation_time
                
                return {
                    'better_port': better_port,
                    'better_score': better_score,
                    'better_ply_data': better_ply_data,
                    'better_prompt': better_prompt,
                    'port1_score': port1_score,
                    'port2_score': port2_score,
                    'comparison_result': f"Port {better_port} wins: {better_score:.4f} vs {port1_score if better_port == self.config['port2'] else port2_score:.4f}"
                }
            else:
                self.logger.error(f"❌ Dual-port validation failed")
                if port1_score is None:
                    self.logger.error(f"   Port {self.config['port1']} validation failed")
                if port2_score is None:
                    self.logger.error(f"   Port {self.config['port2']} validation failed")
                return None
        
        except Exception as e:
            self.logger.error(f"❌ Dual-port validation exception: {e}")
            return None
    
    async def _validate_with_subnet_validator(self, original_prompt: str, generation_prompt: str, ply_data: bytes, port: int) -> Optional[float]:
        """Validate PLY data using subnet validator"""
        try:
            # Save PLY data temporarily
            temp_file = self.output_dir / f"temp_validation_{port}_{int(time.time())}.ply.spz"
            with open(temp_file, 'wb') as f:
                f.write(ply_data)
            
            # Run subnet validator
            cmd = [
                sys.executable,
                ...
                "--endpoint", "generate/",
                "--port", str(port),
                "--pre-generated-ply", str(temp_file)
            ]
            ...
            if result.returncode == 0:
                ...
                if Path(results_file).exists():
                    with open(results_file, 'r') as f:
                        ...
                        self.logger.info(f"   Port {port} validation score: {score:.4f}")
                        return score
                else:
                    self.logger.warning(f"   Port {port} validation results file not found")
                    return None
            else:
                self.logger.warning(f"   Port {port} validation failed (return code {result.returncode})")
                return None
                
        except Exception as e:
            self.logger.error(f"   Port {port} validation error: {e}")
            return None
    
    async def validate_model(self, task: TaskRecord, ply_data: bytes) -> Optional[float]:
        """Validate generated model and update task record (legacy single-port method)"""
        if not self.config['validate_generations']:
            return None
    ...
    async def process_task(self, task: TaskRecord) -> bool:
        """Process a single task end-to-end with dual-port approach"""
        ...
        try:
            # Step 1: Generate 3D model using dual-port approach
            generation_result = await self.generate_3d_model_dual_port(task)
            ...
            # Step 2: Validate both models using subnet validator and compare scores
            validation_result = await self.validate_model_dual_port(task, generation_result)
            if not validation_result:
                self.logger.error(f"❌ Dual-port validation failed for task {task.task_id}")
                self.db.save_task(task)
                return False
            
            # Extract the better result for submission
            better_ply_data = validation_result['better_ply_data']
            better_score = validation_result['better_score']
            better_port = validation_result['better_port']
            
            self.logger.info(f"🏆 Using result from port {better_port} with score {better_score:.4f}")
            
            # Create submission-ready result
            submission_result = {
                'ply_data': better_ply_data,
                'compression_ratio': 'unknown',
                'selected_port': better_port,
                'selected_score': better_score,
                'comparison_result': validation_result['comparison_result']
            }
            ...
            # Step 3: Submit results with the better model
            success = await self.submit_result(task, submission_result)
            
            # Save task record
            self.db.save_task(task)
            
            if success:
                self.logger.info(f"✅ Task {task.task_id} completed successfully using port {better_port}")
            else:
...
async def main():
    ...
    parser.add_argument("--port1", type=int, default=8097, help="Port 1 for optimized prompt generation (default: 8097)")
    parser.add_argument("--port2", type=int, default=8098, help="Port 2 for original prompt generation (default: 8098)")
