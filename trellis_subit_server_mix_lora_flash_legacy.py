...
class TrellisGenerator:
    ...
    def _load_trellis_pipeline(self):
        ...
        try:
            ...
            # from flux_caching import apply_cache_on_pipe
            # apply_cache_on_pipe(self.trellis_pipeline)
...
@app.post("/generate_3d_3view/")
async def generate_3d_from_three_views_endpoint(
    front_image: UploadFile = File(...),
    side_image: UploadFile = File(...),
    back_image: UploadFile = File(...),
    seed: Optional[int] = Form(42),
    ss_guidance_strength: Optional[float] = Form(7.5),
    ss_sampling_steps: Optional[int] = Form(21),
    slat_guidance_strength: Optional[float] = Form(4.0),
    slat_sampling_steps: Optional[int] = Form(24),
    return_compressed: Optional[bool] = Form(True),
    save_preview: Optional[bool] = Form(False),
    remove_background: Optional[bool] = Form(False),
    filter_low_quality: Optional[bool] = Form(True),
    timing: Optional[bool] = Form(False)
):
    """Generate 3D model from three uploaded images using TRELLIS pipeline"""
    
    try:
        print(f"🎯 Starting 3D generation from three views (seed: {seed})")
        start_time = time.time()
        
        # Load and validate images
        print("📸 Loading and validating uploaded images...")
        image_load_start = time.time()
        images = []
        image_names = ["front", "side", "back"]
        uploaded_files = [front_image, side_image, back_image]
        
        for i, (file, name) in enumerate(zip(uploaded_files, image_names)):
            try:
                image_load_single_start = time.time()
                image_data = await file.read()
                pil_image = Image.open(io.BytesIO(image_data))
                
                # Validate image
                if pil_image.size[0] == 0 or pil_image.size[1] == 0:
                    raise ValueError(f"Invalid {name} image dimensions: {pil_image.size}")
                
                image_load_single_time = time.time() - image_load_single_start
                print(f"   ✓ {name} image loaded: {pil_image.size[0]}x{pil_image.size[1]} in {image_load_single_time:.2f}s")
                images.append(pil_image)
                
            except Exception as e:
                raise HTTPException(
                    status_code=400, 
                    detail=f"Failed to load {name} image: {str(e)}"
                )
        
        if len(images) != 3:
            raise HTTPException(status_code=400, detail="Exactly 3 images required")
        
        image_load_total_time = time.time() - image_load_start
        print(f"   Total image loading time: {image_load_total_time:.2f}s")
        
        # Step 1: Remove backgrounds if requested
        if remove_background:
            print("🧹 Removing backgrounds from all three images...")
            ...
            for i, (image, name) in enumerate(zip(images, image_names)):
                try:
                    ...
                    if timing:
                        ...
                        print(f"   ✓ {name} image background removed in {bg_single_time:.2f}s")
                    else:
                        print(f"   ✓ {name} image background removed")
                    cleaned_images.append(cleaned_image)
                except Exception as e:
                    print(f"⚠️ Background removal failed for {name} image: {e}")
                    print("   Continuing with original image...")
                    cleaned_images.append(image)
            
            if timing:
                bg_total_time = time.time() - bg_start_time
                print(f"   Total background removal time: {bg_total_time:.2f}s")
            images = cleaned_images
        else:
            print("   Background removal skipped, using original images")
            bg_total_time = 0.0 if timing else None
        
        # Initialize TRELLIS pipeline if not loaded
        if generator.trellis_pipeline is None:
            print("🔧 Loading TRELLIS pipeline...")
            generator._load_trellis_pipeline()
            if generator.trellis_pipeline is None:
                raise HTTPException(status_code=500, detail="Failed to load TRELLIS pipeline")
        
        # Generate 3D model from multiple views
        print("🎨 Generating 3D model from three views...")
            ...
            try:
                ...
                with torch.autocast(device_type="cuda", dtype=torch.float16):
                    outputs = generator.trellis_pipeline.run_multi_image(
                        ...
                        preprocess_image=True,  # Changed to True to match working pipeline
                        sparse_structure_sampler_params={
                        ...
                        mode="stochastic",  # Use stochastic mode for consistent multi-view generation
                    )
                if timing:
                ...
                with torch.autocast(device_type="cuda", enabled=False):
                    outputs = generator.trellis_pipeline.run_multi_image(
                        images,
                        seed=seed,
                        formats=["gaussian"],  # Only Gaussian splatting
                        preprocess_image=True,  # Changed to True to match working pipeline
                        sparse_structure_sampler_params={
                            "steps": ss_sampling_steps,
                            "cfg_strength": ss_guidance_strength,
        ...
        if timing:
            ...
            print(f"✅ 3D model generated successfully in {trellis_time:.2f}s")
        else:
            print(f"✅ 3D model generated successfully")
        
        # Extract and enhance Gaussian Splatting PLY
        print("🔧 Extracting and enhancing Gaussian Splatting PLY...")
        ply_start_time = time.time() if timing else None
        gaussian_output = outputs['gaussian'][0]
        
        # Quality enhancement: Filter low-quality splats if requested
        ...
        # Compress PLY if requested
        compressed_data = None
        if return_compressed:
            try:
                print("🗜️ Compressing PLY with SPZ...")
        ...
        generation_time = time.time() - start_time
        print(f"🎉 3D generation from three views completed in {generation_time:.2f}s")
        
        if timing:
            print(f"📊 Timing breakdown:")
            print(f"   - Image loading & validation: {image_load_total_time:.2f}s")
            if remove_background:
            ...
            if save_preview and preview_video:
        ...
        if return_compressed and compressed_data:
            return Response(
                ...
                media_type="application/octet-stream",
                headers={
                    "Content-Disposition": f"attachment; filename=three_views_model_{seed}.ply.spz",
                    "X-Generation-Seed": str(seed),
                    "X-Model-Format": "gaussian_splatting_ply",
                    "X-Pipeline": "trellis_multi_image",
                    "X-Compression": "spz",
                    "X-Compression-Ratio": f"{len(compressed_data)/len(ply_data)*100:.1f}%",
        ...
        return Response(
            ...
            media_type="application/octet-stream",
            headers={
                "Content-Disposition": f"attachment; filename=three_views_model_{seed}.ply",
                "X-Generation-Seed": str(seed),
                "X-Model-Format": "gaussian_splatting_ply",
                "X-Pipeline": "trellis_multi_image",
                "X-Compression": "none",
        ...
        print(f"❌ 3D generation from three views failed: {e}")
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"3D generation failed: {str(e)}")

@app.post("/generate_3d_3view_nobg/")
async def generate_3d_from_three_views_nobg_endpoint(
    front_image: UploadFile = File(...),
    side_image: UploadFile = File(...),
    back_image: UploadFile = File(...),
    seed: Optional[int] = Form(42),
    ...
    """Generate 3D model from three uploaded images with background removal using TRELLIS pipeline"""
    
    try:
        print(f"🎯 Starting 3D generation from three views with background removal (seed: {seed})")
        start_time = time.time()
        ...
        # Step 1: Remove backgrounds from all three images
        print("🧹 Removing backgrounds from all three images...")
        bg_start_time = time.time() if timing else None
        if generator.background_remover is None:
            generator._load_background_remover()
        ...
        # Step 2: Initialize TRELLIS pipeline if not loaded
        if generator.trellis_pipeline is None:
            print("🔧 Loading TRELLIS pipeline...")
            generator._load_trellis_pipeline()
            if generator.trellis_pipeline is None:
        ...
        # Step 3: Generate 3D model from cleaned images
        print("🎨 Generating 3D model from cleaned three views...")
        trellis_start_time = time.time() if timing else None
        ...
        if timing:
            trellis_time = time.time() - trellis_start_time
            print(f"✅ 3D model generated successfully from cleaned images in {trellis_time:.2f}s")
        else:
            print(f"✅ 3D model generated successfully from cleaned images")
        ...
        generation_time = time.time() - start_time
        print(f"🎉 3D generation from cleaned three views completed in {generation_time:.2f}s")
        ...
        response_data = {
            ...
            "ply_size_bytes": len(ply_data),
            "model_format": "gaussian_splatting_ply",
            "pipeline": "trellis_multi_image_nobg",
            "image_sizes": [f"{img.size[0]}x{img.size[1]}" for img in cleaned_images],
            "background_removal": "applied",
            "quality_filtering": filter_low_quality
        }
        ...
        if return_compressed and compressed_data:
            return Response(
                ...
                media_type="application/octet-stream",
                headers={
                    "Content-Disposition": f"attachment; filename=three_views_nobg_{seed}.ply.spz",
                    "X-Generation-Seed": str(seed),
                    "X-Model-Format": "gaussian_splatting_ply",
                    "X-Pipeline": "trellis_multi_image_nobg",
                    "X-Compression": "spz",
                    "X-Compression-Ratio": f"{len(compressed_data)/len(ply_data)*100:.1f}%",
                    "X-Generation-Time": f"{generation_time:.2f}s",
                    "X-Response-Data": json.dumps(response_data)
        ...
        return Response(
            ...
            media_type="application/octet-stream",
            headers={
                "Content-Disposition": f"attachment; filename=three_views_nobg_{seed}.ply",
                "X-Generation-Seed": str(seed),
                "X-Model-Format": "gaussian_splatting_ply",
                "X-Pipeline": "trellis_multi_image_nobg",
                "X-Compression": "none",
        ...
        print(f"❌ 3D generation from cleaned three views failed: {e}")
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"3D generation with background removal failed: {str(e)}")

@app.post("/generate_image_img2img/")
async def generate_image_img2img_endpoint(
    prompt: str = Form(...),
    image: UploadFile = File(...),
    strength: Optional[float] = Form(0.8),
    seed: Optional[int] = Form(None),
    num_inference_steps: Optional[int] = Form(8),
    guidance_scale: Optional[float] = Form(3.5)
):
    """Generate image using FLUX image-to-image pipeline."""
    ...
    try:
        ...
        # Convert PIL image to numpy for FLUX pipeline
        input_numpy = np.array(input_image)
        
        print(f"🎨 Generating image-to-image with FLUX for: '{prompt}' (seed: {seed}, strength: {strength})")
        seed_generator = torch.Generator(device=GENERATION_CONFIG['device']).manual_seed(seed)
        ...
        with torch.no_grad():
            ...
            output_image = flux_output.images[0]
...
@app.post("/generate_3d_from_prompt_three_views/")
async def generate_3d_from_prompt_three_views_endpoint(
    ...
    """Generate three views from prompt first, then generate 3D model using TRELLIS pipeline"""
    
    try:
        print(f"🎯 Starting 3D generation from prompt with three view generation (seed: {seed})")
        start_time = time.time()
        
        # Import functions from generate_three_views_server.py
        import sys
        import os
        sys.path.append(os.path.join(os.path.dirname(__file__), 'TRELLIS-TextoImagen3D'))
        from generate_three_views_server import create_view_prompts
        
        # Step 1: Create view-specific prompts
        print("📝 Creating view-specific prompts...")
        prompt_start_time = time.time() if timing else None
        view_prompts = create_view_prompts(base_prompt)
        if timing:
            prompt_time = time.time() - prompt_start_time
            print(f"   ✓ View prompts created in {prompt_time:.2f}s")
        else:
            print("   ✓ View prompts created")
        
        # Step 2: Generate three views using FLUX
        print("🎨 Generating three views using FLUX...")
        image_gen_start_time = time.time() if timing else None
        generated_images = {}
        image_names = ["front", "side", "back"]
        view_seed = seed + hash("front") % 1000
        for view_name, prompt in view_prompts.items():
            try:
                print(f"   Generating {view_name} view...")
                single_image_start = time.time() if timing else None
                
                # Use different seeds for each view to ensure variety while maintaining consistency
                ...
                flux_output = generator.flux_pipeline(
                    ...
                    seed=view_seed,
                    guidance_scale=guidance_scale,
                    num_inference_steps=NUM_INFERENCE_STEPS,  # Use same as generate_three_views_server.py
                    width=width,
                    height=height
                )
                ...
                if image is None or image.size[0] == 0 or image.size[1] == 0:
                    raise RuntimeError(f"Failed to generate valid {view_name} view image")
                
                generated_images[view_name] = image
                
                if timing:
                    single_image_time = time.time() - single_image_start
                    print(f"   ✓ {view_name} view generated ({image.size[0]}x{image.size[1]}) in {single_image_time:.2f}s")
                else:
                    print(f"   ✓ {view_name} view generated ({image.size[0]}x{image.size[1]})")
                    
            except Exception as e:
                raise HTTPException(
                    status_code=500, 
                    detail=f"Failed to generate {view_name} view: {str(e)}"
                )
        
        if timing:
            image_gen_total_time = time.time() - image_gen_start_time
            print(f"   Total image generation time: {image_gen_total_time:.2f}s")
        
        # Step 3: Remove backgrounds from all three images
        print("🧹 Removing backgrounds from all three generated images...")
        bg_start_time = time.time() if timing else None
        if generator.background_remover is None:
            generator._load_background_remover()
        
        cleaned_images = []
        for i, (image, name) in enumerate(zip(generated_images.values(), image_names)):
            try:
                print(f"   Removing background from {name} image...")
                bg_single_start = time.time() if timing else None
                cleaned_image = generator.background_remover(image)
        ...
        # Step 4: Initialize TRELLIS pipeline if not loaded
        if generator.trellis_pipeline is None:
            print("🔧 Loading TRELLIS pipeline...")
            generator._load_trellis_pipeline()
            if generator.trellis_pipeline is None:
        ...
        # Step 5: Generate 3D model from generated and cleaned images
        print("🎨 Generating 3D model from generated three views...")
        trellis_start_time = time.time() if timing else None
        ...
        if timing:
            trellis_time = time.time() - trellis_start_time
            print(f"✅ 3D model generated successfully from generated views in {trellis_time:.2f}s")
        else:
            print(f"✅ 3D model generated successfully from generated views")
        
        # Step 6: Extract and enhance Gaussian Splatting PLY
        print("🔧 Extracting and enhancing Gaussian Splatting PLY...")
        ply_start_time = time.time() if timing else None
        gaussian_output = outputs['gaussian'][0]
        ...
        generation_time = time.time() - start_time
        print(f"🎉 3D generation from prompt with three view generation completed in {generation_time:.2f}s")
        
        if timing:
            print(f"📊 Timing breakdown:")
            print(f"   - Prompt creation: {prompt_time:.2f}s")
            print(f"   - Image generation: {image_gen_total_time:.2f}s")
            print(f"   - Background removal: {bg_total_time:.2f}s")
            print(f"   - TRELLIS pipeline: {trellis_time:.2f}s")
            print(f"   - PLY processing: {ply_total_time:.2f}s")
            if save_preview and preview_video:
        ...
        response_data = {
            ...
            "ply_size_bytes": len(ply_data),
            "model_format": "gaussian_splatting_ply",
            "pipeline": "prompt_to_three_views_to_3d",
            "image_sizes": [f"{img.size[0]}x{img.size[1]}" for img in cleaned_images],
            "background_removal": "applied",
            "quality_filtering": filter_low_quality
        }
        ...
        if return_compressed and compressed_data:
            return Response(
                ...
                media_type="application/octet-stream",
                headers={
                    "Content-Disposition": f"attachment; filename=prompt_three_views_{seed}.ply.spz",
                    "X-Generation-Seed": str(seed),
                    "X-Model-Format": "gaussian_splatting_ply",
                    "X-Pipeline": "prompt_to_three_views_to_3d",
                    "X-Compression": "spz",
                    "X-Compression-Ratio": f"{len(compressed_data)/len(ply_data)*100:.1f}%",
                    "X-Generation-Time": f"{generation_time:.2f}s",
                    "X-Response-Data": json.dumps(response_data)
        ...
        return Response(
            ...
            media_type="application/octet-stream",
            headers={
                "Content-Disposition": f"attachment; filename=prompt_three_views_{seed}.ply",
                "X-Generation-Seed": str(seed),
                "X-Model-Format": "gaussian_splatting_ply",
                "X-Pipeline": "prompt_to_three_views_to_3d",
                "X-Compression": "none",
        ...
        print(f"❌ 3D generation from prompt with three view generation failed: {e}")
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"3D generation from prompt with three view generation failed: {str(e)}")
        ...
        #                 preprocess_image=False,
        #         preprocess_image=False,
        # print("✅ 3D model generated successfully from cleaned images")
        # print("   Enhancing quality by filtering low-quality splats...")
        # try:
        #     points = gaussian_output.points
        #     opacities = gaussian_output.opacities
        ...
        # # import io
        # print(f"✓ Gaussian Splatting PLY extracted ({len(ply_data):,} bytes)")
        #         print(f"✓ Preview video generated ({len(preview_video):,} bytes)")
        #         print(f"✓ SPZ Compression successful:")




# @app.post("/generate_3d_3view_nobg/cinema/")
# async def generate_3d_from_three_views_nobg_cinema_endpoint(
#     front_image: UploadFile = File(...),
#     side_image: UploadFile = File(...),
#     back_image: UploadFile = File(...),
#     seed: Optional[int] = Form(42),
...
#     filter_low_quality: Optional[bool] = Form(True)
# ):
#     """Generate 3D model from three uploaded images with background removal using TRELLIS pipeline and Cinema LoRA"""
    
#     try:
#         print(f"🎯 Starting 3D generation from three views with background removal and Cinema LoRA (seed: {seed})")
#         start_time = time.time()
...
#                 print(f"   ✓ {name} image loaded: {pil_image.size[0]}x{pil_image.size[1]}")
#                 images.append(pil_image)
...
#         # Step 2: Load Cinema LoRA
#         print("🎬 Loading Cinema LoRA...")
#         success = generator._load_lora('cinema')
#         if not success:
#             raise HTTPException(status_code=500, detail="Failed to load Cinema LoRA")
        
#         # Step 3: Initialize TRELLIS pipeline if not loaded
#         if generator.trellis_pipeline is None:
#             print("🔧 Loading TRELLIS pipeline...")
#             generator._load_trellis_pipeline()
#             if generator.trellis_pipeline is None:
...
#         # Step 4: Generate 3D model from cleaned images
#         print("🎨 Generating 3D model from cleaned three views with Cinema LoRA...")
...
#         print("✅ 3D model generated successfully from cleaned images with Cinema LoRA")
...
#         generation_time = time.time() - start_time
#         print(f"🎉 3D generation from cleaned three views with Cinema LoRA completed in {generation_time:.2f}s")
...
#             "ply_size_bytes": len(ply_data),
#             "model_format": "gaussian_splatting_ply",
#             "pipeline": "trellis_multi_image_nobg_cinema",
#             "image_sizes": [f"{img.size[0]}x{img.size[1]}" for img in cleaned_images],
#             "background_removal": "applied",
#             "quality_filtering": filter_low_quality,
#             "lora": "cinema"
...
#                 media_type="application/octet-stream",
#                 headers={
#                     "Content-Disposition": f"attachment; filename=three_views_nobg_cinema_{seed}.ply.spz",
#                     "X-Generation-Seed": str(seed),
#                     "X-Model-Format": "gaussian_splatting_ply",
#                     "X-Pipeline": "trellis_multi_image_nobg_cinema",
#                     "X-Compression": "spz",
#                     "X-Compression-Ratio": f"{len(compressed_data)/len(ply_data)*100:.1f}%",
#                     "X-Generation-Time": f"{generation_time:.2f}s",
#                     "X-Response-Data": json.dumps(response_data)
...
#             media_type="application/octet-stream",
#             headers={
#                 "Content-Disposition": f"attachment; filename=three_views_nobg_cinema_{seed}.ply",
#                 "X-Generation-Seed": str(seed),
#                 "X-Model-Format": "gaussian_splatting_ply",
#                 "X-Pipeline": "trellis_multi_image_nobg_cinema",
#                 "X-Compression": "none",
...
#     except Exception as e:
#         print(f"❌ 3D generation from cleaned three views with Cinema LoRA failed: {e}")
#         traceback.print_exc()
#         raise HTTPException(status_code=500, detail=f"3D generation with background removal and Cinema LoRA failed: {str(e)}")
