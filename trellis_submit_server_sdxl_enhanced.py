#!/usr/bin/env python3
"""
Subnet 17 (404-GEN) - SDXL + TRELLIS Generation Server
Purpose: HTTP server for high-quality 3D model generation using SDXL and TRELLIS
Produces validation-compatible Gaussian Splatting PLY files with SPZ compression

Text Prompt → SDXL Image → TRELLIS 3D → Gaussian Splatting PLY + SPZ Compression
...
# Configuration
GENERATION_CONFIG = {
    'output_dir': './trellis_sdxl_outputs',
...
class GenerationAsset:
    ...
    def add_asset(self, asset_type: AssetType, data: Any):
        ...
        if asset_type == AssetType.SDXL_IMAGE:
            file_path = self.asset_directory / "sdxl_image.png"
            data.save(file_path)
...
class SDXLTrellisGenerator:
    def __init__(self):
        ...
        print("🔧 SDXL + TRELLIS Generator initialized")
        self.ready = True
    ...
    def _load_lora(self, lora_key: str):
        """Load a specific LoRA onto the SDXL pipeline"""
        if self.sdxl_pipeline is None:
            print("❌ SDXL pipeline not loaded. Load SDXL models first.")
            return False
            
        if lora_key not in SDXL_LORAS:
            print(f"❌ LoRA '{lora_key}' not found in available LoRAs")
            return False
            
        lora_config = SDXL_LORAS[lora_key]
        print(f"🔧 Loading LoRA: {lora_config['name']}")
        
        try:
            # Unload any existing LoRA first
            if hasattr(self.sdxl_pipeline, 'unload_lora_weights'):
                self.sdxl_pipeline.unload_lora_weights()
            ...
            if 'path' in lora_config:
                ...
                self.sdxl_pipeline.load_lora_weights(lora_path)
            ...
            print(f"✅ LoRA '{lora_config['name']}' loaded successfully")
            return True
    ...
    def _unload_lora(self):
        """Unload current LoRA from SDXL pipeline"""
        if self.sdxl_pipeline is None:
            return
    ...
    def get_available_loras(self) -> Dict[str, Any]:
        ...
        return {
            ...
            for key, config in SDXL_LORAS.items()
        }
    ...
    def generate_3d_model(self, prompt: str, seed: int = 42) -> Optional[Tuple[bytes, Optional[bytes]]]:
        """Generate 3D model from text prompt using SDXL + TRELLIS pipeline"""
        ...
        with self.generation_lock:
            ...
            try:
                print(f"🎯 Starting SDXL + TRELLIS generation for: '{prompt}' (seed: {seed})")
                ...
                if current_lora and current_lora in SDXL_LORAS:
                    ...
                    if trigger_prefix:
                        ...
                        print(f"🎨 Applied LoRA trigger prefix: '{trigger_prefix}'")
                    else:
                ...
                negative_prompt = "blurry, low quality, distorted, deformed, ugly, bad anatomy, watermark, signature, text, multiple objects, complex background"
                ...
                with torch.no_grad():
                    image = self.sdxl_pipeline(
                        ...
                        guidance_scale=GENERATION_CONFIG['sdxl_guidance_scale'],
                        num_inference_steps=GENERATION_CONFIG['sdxl_num_inference_steps'],
                        width=GENERATION_CONFIG['sdxl_width'],
                        height=GENERATION_CONFIG['sdxl_height'],
                        generator=generator,
                    ).images[0]
                
                print("✓ SDXL image generated successfully")
                generation_asset.add_asset(AssetType.SDXL_IMAGE, image)
                
                # Unload SDXL pipeline to free memory
                self._unload_sdxl_pipeline()
                ...
                if GENERATION_CONFIG.get('enable_object_centering', True):
                    ...
                    try:
                        ...
                        generation_asset.add_asset(AssetType.SDXL_IMAGE, centered_image)
                    except Exception as e:
                        print(f"⚠️ Object centering failed: {e}")
                        print("   Continuing with original image...")
                ...
                print(f"🎉 SDXL + TRELLIS generation completed in {generation_time:.2f}s")
                ...
                print(f"❌ SDXL + TRELLIS generation failed: {e}")
                traceback.print_exc()
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            "status": "running",
            "models_loaded": {
                "sdxl_pipeline": self.sdxl_pipeline is not None,
...
app = FastAPI(title="SDXL + TRELLIS Generation Server", version="1.0.0")

# Initialize global generator
generator = SDXLTrellisGenerator()
...
async def generate_3d_model_endpoint(
    ...
    if return_compressed:
        try:
            ...
            return Response(
                ...
                headers={
                    "Content-Disposition": f"attachment; filename=sdxl_trellis_model_{seed}.ply.spz",
                    "X-Generation-Seed": str(seed),
                    "X-Generation-Prompt": prompt,
                    "X-Model-Format": "gaussian_splatting_ply",
                    "X-Pipeline": "sdxl_trellis",
                    "X-Compression": "spz",
    ...
    return Response(
        ...
        media_type="application/octet-stream",
        headers={
            "Content-Disposition": f"attachment; filename=sdxl_trellis_model_{seed}.ply",
            "X-Generation-Seed": str(seed),
            "X-Generation-Prompt": prompt,
            "X-Model-Format": "gaussian_splatting_ply",
            "X-Pipeline": "sdxl_trellis",
...
async def get_asset_file(asset_type: str):
    ...
    try:
        ...
        filename = f"sdxl_trellis_{asset_type}"
        
        if asset_type_enum == AssetType.SDXL_IMAGE:
...
@app.post("/generate/game_icon/")
async def generate_with_game_icon_lora(
    prompt: str = Form(...), 
    seed: Optional[int] = Form(None),
    return_compressed: Optional[bool] = Form(True)
):
    """Generate 3D model using Game Icon LoRA"""
    try:
        success = generator._load_lora('game_icon')
        if not success:
            raise HTTPException(status_code=500, detail="Failed to load Game Icon LoRA")
        
        result = generator.generate_3d_model(prompt, seed or 42)
        if result is None:
            raise HTTPException(status_code=500, detail="Generation failed")
        ...
        if return_compressed and compressed_data:
            return Response(
                ...
                media_type="application/octet-stream",
                headers={
                    "Content-Disposition": f"attachment; filename=game_icon_{seed or 42}.ply.spz",
        ...
        return Response(
            ...
            media_type="application/octet-stream",
            headers={
                "Content-Disposition": f"attachment; filename=game_icon_{seed or 42}.ply",
                "X-Generation-Seed": str(seed or 42),
                "X-Generation-Prompt": prompt,
                "X-Model-Format": "gaussian_splatting_ply",
                "X-Pipeline": "sdxl_trellis",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SDXL + TRELLIS Generation Server")
    ...
    print(f"Starting SDXL + TRELLIS Generation Server on {args.host}:{args.port}")
    print("=" * 80)
    print("Pipeline: Text → SDXL → Image → TRELLIS → Gaussian Splatting PLY")
    print("Features:")
    print("  • SDXL text-to-image generation for better alignment")
