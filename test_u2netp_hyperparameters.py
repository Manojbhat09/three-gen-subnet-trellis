#!/usr/bin/env python3
"""
Test u2netp hyperparameters and their impact on performance and output quality.
"""
...
def run_single_test(
    ...
    try:
        ...
        # Save result
        if config.only_mask:
            output_filename = f"{image_stem}.u2netp.{config.name}.mask.png"
        else:
            output_filename = f"{image_stem}.u2netp.{config.name}.png"
...
def main():
    parser = argparse.ArgumentParser(
        description="Test u2netp hyperparameters and their impact on performance and quality."
    )
    ...
    parser.add_argument(
        "output_dir",
        nargs="?",
        default="/home/mbhat/three-gen-subnet-trellis/test_u2netp_hyperparams",
    ...
    print(f"Testing u2netp with {len(configs_to_run)} configurations")
