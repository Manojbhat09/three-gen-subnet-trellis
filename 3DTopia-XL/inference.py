...
def main(config):
    ...
    for img_path in img_list:
        ...
        print("Initial array type:", type(raw_image), raw_image.dtype, raw_image.shape)
        mask = (raw_image[..., -1][..., None] > 0) * 1
        raw_image = raw_image[..., :3] * mask
        print("After mask type:", type(raw_image), raw_image.dtype, raw_image.shape)
        # Ensure we have a float32 array with correct shape
        raw_image = raw_image.astype(np.float32)
        print("After float32 type:", type(raw_image), raw_image.dtype, raw_image.shape)
        raw_image = np.expand_dims(raw_image, axis=0)  # Add batch dimension
        print("After expand_dims type:", type(raw_image), raw_image.dtype, raw_image.shape)
        # Convert to tensor directly
        input_cond = torch.tensor(raw_image, dtype=torch.float32).to(device)
        with torch.no_grad():
