#!/usr/bin/env python3
"""
Analyze u2net hyperparameter test results and provide insights.
"""
...
def analyze_hyperparams(csv_path: str):
    ...
    print("🔍 U2NET HYPERPARAMETER ANALYSIS INSIGHTS")
    print("=" * 60)
    ...
    # Speed categories
    fast = successful[successful['elapsed_seconds'] < 5.0]
    medium = successful[(successful['elapsed_seconds'] >= 5.0) & (successful['elapsed_seconds'] < 15.0)]
    slow = successful[successful['elapsed_seconds'] >= 15.0]
    
    print(f"🚀 SPEED CATEGORIES:")
    print(f"   • Fast (<5s): {len(fast)} configurations")
    print(f"   • Medium (5-15s): {len(medium)} configurations")
    print(f"   • Slow (≥15s): {len(slow)} configurations")
    print()
    ...
    balanced = successful[
        (successful['elapsed_seconds'] < 8.0) & 
...
def main():
    csv_path = "/home/mbhat/three-gen-subnet-trellis/test_u2net_hyperparams/summary.csv"
