...
class LLMClosePromptReproducibility:
    ...
    def extract_patterns(self, prompt_to_deconstruct: str) -> Dict[str, Any]:
        ...
        system_prompt = """
You are a prompt analyst for 3D model descriptions. Your singular task is to parse the given text prompt into a JSON object that strictly adheres to JSON syntax rules.

**CRITICAL INSTRUCTIONS - READ CAREFULLY:**
- Output ONLY a valid JSON object with double quotes around ALL property names and string values.
- No extra text, commentary, or formatting besides the exact JSON.
- Use arrays ([]) for lists and empty arrays if nothing applies.
- Keys must be exactly: "core_subject" and "enhancements", with enhancements containing 4 keys: "quality_adjectives", "material_details", "light_interaction", "context".
- If uncertain, output valid JSON with empty lists rather than text or partial output.

**JSON STRUCTURE REQUIRED:**
{
  "core_subject": "The main object being described (e.g., 'sandstone cat sculpture')",
  "enhancements": {
    "quality_adjectives": ["adjectives describing quality like 'intricate', 'detailed', 'rough']",
    "material_details": ["materials and textures like 'sandstone', 'glass', 'metal']",
    "light_interaction": ["light effects like 'glowing', 'shimmering', 'sparkling']",
    "context": ["background or setting details like 'white background', 'studio lighting']"
  }
}

**EXAMPLE INPUT:** "intricate sandstone sculpture of cat lounging"
**EXAMPLE OUTPUT:** {"core_subject":"sandstone cat sculpture","enhancements":{"quality_adjectives":["intricate"],"material_details":["sandstone"],"light_interaction":[],"context":[]}}

**FINAL OUTPUT (ONLY THE JSON, NO EXPLANATION):**"""
    ...
    def reconstruct_prompt(self, components: Dict[str, Any], reference_original: str = None, reference_optimized: str = None, target_original: str = None) -> str:
        ...
        system_prompt = f"""You are a prompt assembly agent for 3D model generation. Your task is to reconstruct a high-quality 3D model prompt from structured components.

**CRITICAL INSTRUCTIONS - READ CAREFULLY:**
1. You MUST start with the exact `core_subject` provided - NEVER change it
2. You MUST integrate the enhancement phrases to describe the subject
3. You MUST end with ", white background"
4. You MUST NOT invent new subjects or change the core subject
5. Study the reference examples below to understand the style and quality we want
...
Notice how the optimized version enhances the original while preserving the core subject and intent.
"""
        
        if target_original:
            system_prompt += f"""Target Original: "{target_original}"

This is what we're trying to enhance. Study its style and structure.
"""
        
        system_prompt += f"""
**STEP-BY-STEP PROCESS:**
1. Take the core_subject: "{components.get('core_subject', '')}"
2. Add quality adjectives: {components.get('enhancements', {}).get('quality_adjectives', [])}
3. Add material details: {components.get('enhancements', {}).get('material_details', [])}
4. Add light effects: {components.get('enhancements', {}).get('light_interaction', [])}
5. Add context: {components.get('enhancements', {}).get('context', [])}
6. End with ", white background"

You are open to be a bit more creative and add details that are not in the original prompt but make it in the same style and quality.
Be creative with adjectives and placements.
Dont add a lot of comma, just use them when necessary.

**STYLE GUIDANCE:**
- Study the reference examples above to understand the quality and style
- Be creative with adjective placement and flow
- Don't overuse commas - use them only when necessary
- Maintain the natural language flow of the reference examples
- If the reference examples use specific patterns (like "with X details" or "featuring Y"), consider similar patterns

**EXAMPLE OUTPUT FORMAT:**
If core_subject is "sandstone cat sculpture" and enhancements include "intricate" and "bubbly", 
output: "intricate bubbly sandstone cat sculpture, white background"

**FINAL OUTPUT (ONLY THE PROMPT, NO EXPLANATION):**"""
