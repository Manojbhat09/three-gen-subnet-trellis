...
class LLaMACustomGenerator:
    ...
    def _build_system_prompt(self, instruction: LLaMAInstruction) -> str:
        base_prompt = f"""You are an expert 3D asset generation & image generation prompt optimizer that creates CUSTOM prompts for maximum scores.
...
3. PRESERVE main object from original - DO NOT change the object type
4. CREATE contextually appropriate descriptions for THIS specific object
5. FOCUS on {instruction.focus_area} optimization that makes sense for the object
6. NO technical terms unless they're appropriate for the object (e.g., no "aerospace-grade" for food/drinks)
...
REASONING: [Why this will score 0.9+]"""
        ...
        if self.successful_examples:
            base_prompt += "\n\nHIGH-SCORING EXAMPLES (use these patterns):"
            # Show top scoring examples first
            for ex in self.successful_examples[:4]:
                base_prompt += f"\n{ex['custom']} (Score: {ex['score']:.3f})"
        
        if self.learned_patterns:
            base_prompt += "\n\nPROVEN PATTERNS (apply these techniques):"
            # Sort patterns by score
            sorted_patterns = sorted(self.learned_patterns, key=lambda x: x['score'], reverse=True)
            for pattern in sorted_patterns[:4]:
                base_prompt += f"\n{pattern['pattern']} (Score: {pattern['score']:.3f})"
            
            base_prompt += "\n\nINSTRUCTION: Incorporate elements from the highest-scoring examples above."

        return base_prompt
    
    def _build_user_prompt(self, original: str, instruction: LLaMAInstruction) -> str:
        return f"""OPTIMIZE ORIGINAL PROMPT: "{original}"
...
Based on the SPECIFIC nature of ORIGINAL PROMPT, create a completely custom optimization that addresses its unique characteristics.
Think about what makes THIS EXACT object special and how to enhance those qualities for 3D generation.

CRITICAL: Your optimization must be APPROPRIATE for ORIGINAL PROMPT. 
Think about what this object IS and enhance it accordingly.
...
# - wbgmsst, defense-grade ultra-precision {original}, premium excellence, white background
    ...
    def learn_from_feedback(self, original: str, custom: str, score: float, strategy: str):
        if score >= 0.8:
            ...
            # Keep more examples, prioritize high scores
            self.successful_examples = sorted(self.successful_examples, key=lambda x: x['score'], reverse=True)[:12]
            print(f"   🧠 LLaMA learned success: {strategy} → {score:.3f}")
            
            # If this is an excellent score, immediately extract its pattern
            if score >= 0.9:
                self._extract_and_store_pattern(custom, score)
    ...
    def _extract_and_store_pattern(self, prompt: str, score: float):
        """Extract pattern from high-scoring prompt immediately"""
        prompt_lower = prompt.lower()
        
        # Look for key technical phrases that led to high scores
        patterns_to_check = [
            ('micro-iridescent.*precision-crafted', 'micro-iridescent precision-crafted'),
            ('precision tolerances', 'precision-tolerance specification'),
            ('microscopic structural gradient', 'microscopic gradient enhancement'),
            ('intricate optical clarity', 'intricate optical clarity'),
            ('nanocrystalline.*enhanced', 'nanocrystalline enhancement'),
            ('optically-graded', 'optically-graded specification')
        ]
        ...
        for pattern_regex, pattern_name in patterns_to_check:
            if re.search(pattern_regex, prompt_lower):
                ...
                existing = [p for p in self.learned_patterns if pattern_name in p['pattern']]
                if not existing:
                    self.learned_patterns.append({'pattern': pattern_name, 'score': score})
                    print(f"   ⚡ IMMEDIATE PATTERN: {pattern_name} (Score: {score:.3f})")
                    break
...
class CustomPromptEnvironmentV3:
    ...
    def log_prompt_score(self, original, custom, score, episode, step):
        # Use the checkpoint directory for the log file
        log_path = Path(self.checkpoint_dir) / "prompt_score_log.csv"
        file_exists = log_path.exists()
        with open(log_path, "a", newline='') as f:
            writer = csv.writer(f)
            # Write header if file is new
            if not file_exists:
                writer.writerow(["episode", "step", "original", "custom", "score"])
            writer.writerow([episode, step, original, custom, score])
    ...
    def step(self, action_idx: int):
        ...
        self.log_prompt_score(self.target_prompt, custom_prompt, new_score, len(self.episode_log), self.step_count)
...
class RLLLaMAOptimizerV3:
    ...
    def _meta_learning_phase(self):
        ...
        # IMPROVED: Analyze ALL high-scoring prompts, not just the best
        high_score_threshold = 0.85
        excellent_prompts = [s for s in recent_successes if s.score_achieved >= high_score_threshold]
        
        if not excellent_prompts:
            print("   📊 No prompts above 0.85 threshold for meta-learning")
            return
        
        print(f"   🎯 Analyzing {len(excellent_prompts)} excellent prompts (≥{high_score_threshold})")
        
        # Sort by score for analysis
        excellent_prompts.sort(key=lambda x: x.score_achieved, reverse=True)
        
        # Extract patterns from top prompts
        patterns_learned = 0
        for prompt_event in excellent_prompts[:5]:  # Analyze top 5
            print(f"   📝 Score {prompt_event.score_achieved:.3f}: {prompt_event.successful_prompt}")
            
            pattern = self._extract_detailed_pattern(prompt_event.original_prompt, prompt_event.successful_prompt)
            if pattern and pattern not in [p['pattern'] for p in self.env.llama_generator.learned_patterns]:
                self.env.llama_generator.learn_pattern_from_meta_learning(pattern, prompt_event.score_achieved)
                self.training_state.new_patterns_learned += 1
                patterns_learned += 1
                print(f"   🎉 NEW PATTERN: {pattern}")
        
        # Also extract common elements across multiple high-scoring prompts
        common_pattern = self._extract_common_patterns(excellent_prompts)
        if common_pattern:
            self.env.llama_generator.learn_pattern_from_meta_learning(common_pattern, 
                                                                    max(p.score_achieved for p in excellent_prompts))
            self.training_state.new_patterns_learned += 1
            patterns_learned += 1
            print(f"   🌟 COMMON PATTERN: {common_pattern}")
        
        print(f"   📚 Total patterns learned this phase: {patterns_learned}")
        print(f"{'='*60}\n")

    def _extract_detailed_pattern(self, original: str, successful: str) -> Optional[str]:
        """Enhanced pattern extraction that captures more specific techniques"""
        successful_lower = successful.lower()

        # Look for specific quality descriptors
        if 'micro-iridescent' in successful_lower and 'precision' in successful_lower:
            return f"micro-iridescent precision-crafted {original}"
        elif 'precision tolerances' in successful_lower:
            return f"precision-tolerance enhanced {original}"
        elif 'microscopic structural gradient' in successful_lower:
            return f"microscopic gradient-enhanced {original}"
        elif 'aerospace-grade' in successful_lower and 'precision' in successful_lower:
        ...
        elif 'nanocrystalline' in successful_lower and 'enhanced' in successful_lower:
            return f"nanocrystalline-enhanced {original}"
        elif 'optically-graded' in successful_lower:
            return f"optically-graded {original}"
        elif 'intricate optical clarity' in successful_lower:
            return f"intricate optical clarity {original}"
        elif 'masterpiece-quality' in successful_lower:
            return f"masterpiece-quality {original}"
        
        return None

    def _extract_common_patterns(self, excellent_prompts: List) -> Optional[str]:
        """Find common elements across multiple high-scoring prompts"""
        if len(excellent_prompts) < 2:
            return None
        
        # Count common terms across prompts
        all_prompts = [p.successful_prompt.lower() for p in excellent_prompts]
        common_terms = []
        
        # Look for technical terms that appear in multiple high-scoring prompts
        technical_terms = [
            'precision-crafted', 'optical clarity', 'microscopic', 'structural',
            'enhanced', 'grade', 'iridescent', 'tolerances', 'intricate'
        ]
        
        for term in technical_terms:
            count = sum(1 for prompt in all_prompts if term in prompt)
            if count >= len(excellent_prompts) * 0.6:  # Appears in 60%+ of prompts
                common_terms.append(term)
        
        if common_terms:
            # Use the original prompt from the best scoring example
            best_original = max(excellent_prompts, key=lambda x: x.score_achieved).original_prompt
            return f"{'-'.join(common_terms[:2])} {best_original}"
    ...
    def train_with_checkpoints(self, target_prompts: List[str], 
        ...
        while True:  # Continuous learning loop
            # IMPROVED: Check if we should start a new cycle based on episode count
            total_episodes_for_complete_cycle = total_prompts * episodes_per_prompt
            
            # If we're starting a new episode (episode + 1), check where we should be
            next_episode = self.training_state.episode + 1
            episode_in_cycle = (next_episode - 1) % total_episodes_for_complete_cycle
            expected_prompt_idx = episode_in_cycle // episodes_per_prompt
            

            
            # If we've completed a full cycle or are in wrong position, correct it
            if prompt_idx != expected_prompt_idx:
                if continuous_learning:
                    print(f"\n🔄 CYCLE DETECTION: Episode {self.training_state.episode}")
                    print(f"   📊 Expected prompt: {expected_prompt_idx + 1}, Current: {prompt_idx + 1}")
                    
                    if next_episode > total_episodes_for_complete_cycle and episode_in_cycle == 0:
                        print(f"   🔄 COMPLETED FULL CYCLE! (Episodes 1-{total_episodes_for_complete_cycle})")
                        print(f"   📈 Current Knowledge: ε={self.agent.epsilon:.3f}, Patterns={self.training_state.new_patterns_learned}")
                        print(f"   💾 All learning preserved (checkpoint, weights, patterns)")
                        print()
                        
                        choice = input("   ➡️ Continue training with NEW CYCLE from prompt 1? (Y/n): ").strip().lower()
                        if choice in ['n', 'no']:
                            print("   🛑 Stopping continuous learning as requested")
                            return self._generate_final_report()
                        
                        # Ask if they want to change episodes per prompt for new cycle
                        change_episodes = input(f"   🔧 Change episodes per prompt? (currently {episodes_per_prompt}) (y/N): ").strip().lower()
                        if change_episodes in ['y', 'yes']:
                            try:
                                new_episodes = int(input(f"   📊 Enter new episodes per prompt: "))
                                if new_episodes > 0:
                                    episodes_per_prompt = new_episodes
                                    self.training_state.episodes_per_prompt = new_episodes
                                    print(f"   ✅ Updated to {new_episodes} episodes per prompt")
                                    # Recalculate cycle with new episode count
                                    total_episodes_for_complete_cycle = total_prompts * episodes_per_prompt
                                else:
                                    print(f"   ❌ Invalid number, keeping {episodes_per_prompt}")
                            except ValueError:
                                print(f"   ❌ Invalid input, keeping {episodes_per_prompt}")
                        
                        print(f"   🆕 STARTING NEW CYCLE from prompt 1 (Episodes: {episodes_per_prompt} each)")
                    else:
                        print(f"   🔄 CORRECTING: Moving to correct prompt position (within current cycle)")
                    
                    prompt_idx = expected_prompt_idx
