...
def load_base_unet(pretrained_model_name_or_path, **kwargs):
    """Load the base UNet2DConditionModel from pretrained weights."""
...
class UNet2p5DConditionModel(torch.nn.Module):
    ...
    def from_pretrained(pretrained_model_name_or_path, **kwargs):
        ...
        # Load base UNet
        base_unet = load_base_unet(pretrained_model_name_or_path, **kwargs)
        
        # Create 2.5D UNet wrapper
        unet = UNet2p5DConditionModel(base_unet)
        
        # Load additional 2.5D weights if available
        unet.load_2p5d_weights(pretrained_model_name_or_path)
        
        return unet

    def load_2p5d_weights(self, pretrained_model_name_or_path):
        """Load additional weights for 2.5D components."""
        weights_path = os.path.join(pretrained_model_name_or_path, '2p5d_weights.bin')
        if os.path.exists(weights_path):
            weights = torch.load(weights_path, map_location='cpu')
            # Load only the 2.5D specific weights
            model_dict = self.state_dict()
            pretrained_dict = {k: v for k, v in weights.items() if k in model_dict and 'learned_text_clip' in k}
