...
torch.backends.cudnn.deterministic = False    # For reproducibility with cuDNN
...
# Nunchaku integration via HTTP API
try:
    from nunchaku_client import generate_nunchaku_image_http
    NUNCHAKU_AVAILABLE = True
    print("✅ Nunchaku HTTP client imported successfully")
except ImportError as e:
    print(f"⚠️ Nunchaku HTTP client not available: {e}")
...
GENERATION_CONFIG = {
    ...
    'current_model': 'nunchaku_flux',  # 'flux', 'sdxl', or 'sd15'
    ...
    'trellis_compile_mode': 'default',  # options: 'reduce-overhead', 'max-autotune' (if supported)
    ...
    'nunchaku_socket_path': '/home/mbhat/three-gen-subnet-trellis/inferences_nunchaku.sock',
}
...
# SDXL LoRA definitions (empty for now, as no specific SDXL LoRAs are defined)
SDXL_LORAS = {}
...
class NunchakuFluxSocketClient:
    """Client for communicating with isolated Nunchaku FLUX inference server"""

    def __init__(self, socket_path="/home/mbhat/three-gen-subnet-trellis/inferences_nunchaku.sock"):
        self.socket_path = socket_path
        self.connection = None
    ...
    def _ensure_connection(self):
        ...
        try:
            ...
            print(f"⚠️ Failed to connect to Nunchaku FLUX socket: {e}")
            return False
        return True
    ...
    def generate_image(self, prompt: str, seed: int, width: int = 1024, height: int = 1024, **kwargs) -> Optional[Image.Image]:
        ...
        try:
            ...
            print(f"❌ Socket-based Nunchaku FLUX generation failed: {e}")
            return None
...
class TrellisGenerator:
    def __init__(self):
        ...
        self.nunchaku_flux_socket_client = None
        ...
        if GENERATION_CONFIG.get('current_model', 'flux') == 'nunchaku_flux':
            self._start_nunchaku_flux_server()
    ...
    def _start_nunchaku_flux_server(self):
        """Start Nunchaku FLUX server as subprocess"""
        try:
            print("🚀 Starting Nunchaku FLUX server...")
            nunchaku_dir = os.path.join(os.path.dirname(__file__), "..", "nunchaku_schnell")
            # Ensure the correct conda environment is activated
            command = f"conda run -n nun python {os.path.join(nunchaku_dir, 'main.py')}"
            self.nunchaku_flux_process = subprocess.Popen(
                command,
                shell=True,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                executable='/bin/bash' # Specify bash to ensure conda activate works
            )
            
            # Wait for server to be ready
            time.sleep(10) # Increased sleep time for conda environment activation
            print("✅ Nunchaku FLUX server started")
            
        except Exception as e:
            print(f"❌ Failed to start Nunchaku FLUX server: {e}")
            traceback.print_exc()
            self.nunchaku_flux_process = None
    ...
    def _load_flux_models(self):
        """Load FLUX models"""
        if self.flux_pipeline is not None and GENERATION_CONFIG.get('current_model') != 'nunchaku_flux':
            print("✓ FLUX models already loaded")
            return
            
        if self.nunchaku_flux_socket_client is not None and GENERATION_CONFIG.get('current_model') == 'nunchaku_flux':
            print("✓ Nunchaku FLUX socket client already initialized")
            return
        ...
        try:
            ...
            if GENERATION_CONFIG.get('current_model') == 'nunchaku_flux':
                try:
                    print("🔧 Initializing Nunchaku FLUX socket client...")
                    self.nunchaku_flux_socket_client = NunchakuFluxSocketClient(
                        socket_path=GENERATION_CONFIG.get('nunchaku_socket_path')
                    )
                    
                    # Test connection with a simple generation
                    test_image = self.nunchaku_flux_socket_client.generate_image(
                        prompt="test", 
                        seed=42, 
                        width=512, 
                        height=512
                    ...
                    if test_image is not None:
                        print("✅ Nunchaku FLUX socket client initialized successfully")
                        return
                    else:
                        print("⚠️ Nunchaku FLUX socket client failed, falling back to direct loading")
                        self.nunchaku_flux_socket_client = None
                        
                except Exception as e:
                    print(f"⚠️ Nunchaku FLUX socket client failed: {e}")
                    self.nunchaku_flux_socket_client = None
            ...
            if GENERATION_CONFIG.get('current_model') == 'nunchaku_flux':
                try:
                    print("🔧 Warming up Nunchaku FLUX socket client...")
                    for _ in range(2):
                        gc.collect()
                        with torch.no_grad():
                            _ = self.nunchaku_flux_socket_client.generate_image(
                                prompt="",
                                width=1024,
                                height=1024,
                            )
                        print("✓ Nunchaku FLUX socket client warmup complete")
                except Exception as we:
                    print(f"⚠️ Nunchaku FLUX socket client warmup skipped: {we}")
    ...
    def _unload_flux_models(self):
        ...
        if self.nunchaku_flux_socket_client is not None:
            # Assuming the socket client has a close method or similar for cleanup
            # If not, simply deleting the reference is sufficient
            del self.nunchaku_flux_socket_client
            self.nunchaku_flux_socket_client = None
            models_unloaded.append("nunchaku_flux_socket_client")
    ...
    def _unload_current_model(self):
        ...
        print(f"🧹 Unloading current model: {current_model.upper()}...")

        if current_model == 'flux' or current_model == 'nunchaku_flux':
            self._unload_flux_models()
        elif current_model == 'sdxl':
            self._unload_sdxl_pipeline()
        elif current_model == 'sd15':
            self._unload_sd15_pipeline()
        elif current_model == 'hunyuan':
            self._unload_hunyuan_pipeline()
        print(f"✅ {current_model.upper()} model unloaded.")

    def generate_nunchaku_image(self, prompt: str, seed: int = 42, width: int = 1024, height: int = 1024):
        """Generate image using Nunchaku via subprocess"""
        if not NUNCHAKU_AVAILABLE:
            print("❌ Nunchaku subprocess integration not available")
            return None
        
        try:
            return generate_nunchaku_image_http(prompt, seed, width, height)
        except Exception as e:
            print(f"❌ Nunchaku HTTP client failed: {e}")
            return None
    ...
    def generate_3d_model(self, prompt: str, seed: int = 42, num_inference_steps: Optional[int] = None, guidance_scale: Optional[float] = None, ss_sampling_steps: Optional[int] = None, slat_sampling_steps: Optional[int] = None, slat_guidance_strength: Optional[float] = None, ss_guidance_strength: Optional[float] = None) -> Optional[Tuple[bytes, Optional[bytes]]]:
        ...
        with self.generation_lock:
            ...
            try:
                ...
                if current_model == 'flux':
                ...
                elif current_model == 'nunchaku_flux':
                    # Use direct Nunchaku integration instead of socket
                    print("�� Using direct Nunchaku integration for image generation...")
                    
                    image = self.generate_nunchaku_image(
                        prompt=enhanced_prompt,
                        seed=seed,
                        width=1024,
                        height=1024
                    ...
                    if image is None:
                        raise RuntimeError("Nunchaku image generation failed.")
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "models_loaded": {
                ...
                "nunchaku_flux_socket_client": self.nunchaku_flux_socket_client is not None,
            },
            "metrics": {
...
async def switch_model(
    ...
    """Switch between different models (FLUX, SDXL, SD1.5, Nunchaku-FLUX)"""
    try:
        if model not in ['flux', 'sdxl', 'sd15', 'nunchaku_flux']:
            return JSONResponse(content={
                "status": "error",
                "message": "Invalid model. Must be 'flux', 'sdxl', 'sd15', or 'nunchaku_flux'"
            }, status_code=400)
        ...
        # Unload current base model if any
        generator._unload_current_model()
...
async def get_current_model():
    ...
    try:
        return {
            "status": "success",
            "current_model": GENERATION_CONFIG.get('current_model', 'flux'),
            "available_models": ['flux', 'sdxl', 'sd15', 'nunchaku_flux']
        }
    except Exception as e:
        return JSONResponse(content={
            "status": "error",
...
async def generate_image_endpoint(
    ...
    try:
        ...
        if current_model == 'flux':
            ...
            # Use direct Nunchaku integration
            print(f"🎨 Generating image with Nunchaku for: '{prompt}' (seed: {seed})")
            
            image = generator.generate_nunchaku_image(
                prompt=prompt,
                seed=seed,
                width=1024,
                height=1024
            ...
            if image is None:
                raise HTTPException(status_code=500, detail="Nunchaku image generation failed")
