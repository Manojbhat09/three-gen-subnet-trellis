'''
python enhance_3d_optim.py outputs/wbgmsst,_a_blue_monkey_sitting_on_temple,_3d_isometric,_white_background.glb --output-dir enhanced_outputs

python enhance_3d_optim.py outputs/wbgmsst,_a_blue_monkey_sitting_on_temple,_3d_isometric,_white_background.glb \
...
def result_mesh(mesh=None, vertices=None, faces=None, run_dir=".", name="test"):
    # Generate preview videos for the original model
    print("\nGenerating preview videos for model...")
    ...
    video = render_utils.render_video(original_mesh)['normal']
    imageio.mimsave(str(run_dir / f"preview_{name}_mesh.mp4"), video, fps=30)
    print("mesh result saved")
    

def load_mesh(mesh_path, num_pc=20480, dtype=torch.float16, run_dir=None):
    ...
    if run_dir:
        result_mesh(mesh, run_dir=run_dir, name="original")
...
def enhance_3d_model(input_glb_path, output_dir="outputs", octree_depth=9, num_inference_steps=50, guidance_scale=10.0):
    ...
    surface = load_mesh(input_glb_path, dtype=dtype, run_dir=run_dir)
    ...
    result_mesh(vertices=vertices, faces=faces, run_dir=run_dir, name="enhanced")
...
def main():
    ...
    parser.add_argument("--octree-depth", type=int, default=9, help="Depth of octree for point sampling")
