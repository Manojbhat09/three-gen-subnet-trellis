...
'''
python generate_3d_bg.py "a blue monkey sitting on temple" --guidance-scale 3.5 --ss-guidance-strength 7.5 --ss-sampling-steps 12 --slat-guidance-strength 3.0 --slat-sampling-steps 12
python generate_3d_bg.py "football" --guidance-scale 3.5 --ss-guidance-strength 7.5 --ss-sampling-steps 12 --slat-guidance-strength 3.0 --slat-sampling-steps 12
...
class BackgroundRemover:
    """BackgroundRemover class matching hy3dgen.rembg.BackgroundRemover implementation"""
    def __init__(self):
        if REMBG_AVAILABLE:
    ...
    def __call__(self, image):
        ...
        try:
            # Use the same parameters as hy3dgen.rembg.BackgroundRemover
            output = rembg.remove(image, session=self.session, bgcolor=[255, 255, 255, 0])
            return output
...
def remove_background_custom(image):
    """Custom background removal using simple thresholding (fallback)"""
    try:
        ...
        # Simple white background removal (adjust threshold as needed)
        # This is a very basic approach - assumes white/light background
        mask = np.all(img_array[:, :, :3] > 240, axis=2)
        img_array[mask, 3] = 0  # Set alpha to 0 for background pixels
        
        return Image.fromarray(img_array)
    except Exception as e:
        print(f"Error with custom background removal: {e}")
        return image

def generate_3d_model_with_bg_removal(prompt, output_dir="outputs", seed=42, width=1024, height=1024, guidance_scale=3.5,
                                     ss_guidance_strength=7.5, ss_sampling_steps=12,
                                     slat_guidance_strength=3.0, slat_sampling_steps=12,
                                     bg_removal_method="rembg"):
    """
    Generate a 3D model from a text prompt using Flux and TRELLIS with background removal.
    ...
    Args:
        ...
        bg_removal_method (str): Background removal method ("rembg" or "custom")
    """
    ...
    # Use white background optimized prompt for better background removal
    prompt_with_bg = "wbgmsst, " + prompt + ", 3D isometric object asset, clean white background, isolated object"
    ...
    original_image_path = output_path / "generated_image_original.png"
    image.save(original_image_path)
    print(f"Saved original generated image to {original_image_path}")
    
    # Remove background using hy3dgen-style BackgroundRemover
    print("Removing background...")
    rembg = BackgroundRemover()
    if bg_removal_method == "rembg" and REMBG_AVAILABLE:
        image_no_bg = rembg(image)
    else:
        image_no_bg = remove_background_custom(image)
    
    # Save the background-removed image
    no_bg_image_path = output_path / "generated_image_no_bg.png"
    ...
    # Generate 3D model from background-removed image
    print("Generating 3D model from background-removed image...")
    outputs = trellis_pipeline.run(
        image_no_bg,  # Use background-removed image
        seed=seed,
    ...
    model_name = prompt.replace(" ", "_").replace(",", "").lower()
    ...
    print("- generated_image_original.png (Original 2D image)")
    print("- generated_image_no_bg.png (Background-removed image)")
...
def main():
    parser = argparse.ArgumentParser(description="Generate 3D models from text prompts using Flux and TRELLIS with background removal")
    ...
    parser.add_argument("--bg-removal-method", type=str, default="rembg", choices=["rembg", "custom"], 
                        help="Background removal method (rembg or custom)")
    ...
    generate_3d_model_with_bg_removal(
        args.prompt,
        args.output_dir,
        args.seed,
        args.width,
