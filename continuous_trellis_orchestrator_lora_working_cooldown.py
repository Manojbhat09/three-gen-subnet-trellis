    ...
    def save_validator_states(self, validators: Dict[int, 'ValidatorState']) -> bool:
        ...
        try:
            ...
            state_data = {
                ...
                'version': '2.0',  # Simplified cooldown system
                'validators': {}
            }
        ...
        try:
            ...
            for uid_str, validator_data in state_data['validators'].items():
                try:
                    ...
                    if validator_data.get('validator_enforced_cooldown_until'):
                        if current_time >= validator_data['validator_enforced_cooldown_until']:
                            validator_data['validator_enforced_cooldown_until'] = None
                            expired_cooldowns += 1
                    
                    if validator_data.get('miner_cooldown_until'):
                        if current_time >= validator_data['miner_cooldown_until']:
                            validator_data['miner_cooldown_until'] = None
                            expired_cooldowns += 1
...
class TaskDatabase:
    ...
    def _migrate_database_if_needed(self, cursor):
        ...
        try:
            # Check if old cooldown_until column exists
            cursor.execute("PRAGMA table_info(validators)")
            columns = [column[1] for column in cursor.fetchall()]
            
            if 'cooldown_until' in columns and 'validator_enforced_cooldown_until' not in columns:
                # Migrate old schema to new schema
                cursor.execute('''
                    ALTER TABLE validators 
                    ADD COLUMN validator_enforced_cooldown_until REAL
                ''')
                cursor.execute('''
                    ALTER TABLE validators 
                    ADD COLUMN miner_cooldown_until REAL
                ''')
                
                # Copy old cooldown data to new fields
                cursor.execute('''
                    UPDATE validators 
                    SET validator_enforced_cooldown_until = cooldown_until,
                        miner_cooldown_until = cooldown_until
                    WHERE cooldown_until IS NOT NULL
                ''')
                
                # Remove old column (SQLite doesn't support DROP COLUMN, so we'll recreate the table)
                # For now, we'll keep the old column but mark it as deprecated
                print("✅ Database migrated from old schema to new schema")
    ...
    def init_database(self):
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS validators (
                ...
                validator_enforced_cooldown_until REAL,
                miner_cooldown_until REAL,
...
class ContinuousTrellisOrchestrator:
    ...
    def is_validator_available(self, validator: ValidatorState) -> bool:
        ...
        # Check if cooldowns are globally disabled
        if self.config.get('disable_all_cooldowns', False):
            self.logger.debug(f"🚫 Cooldowns disabled - validator UID {validator.uid} is always available")
            return True
        ...
        # Simplified cooldown checking using helper method
    ...
    def set_validator_cooldown(self, validator: ValidatorState, cooldown_seconds: int, reason: str):
        """
        Simplified cooldown management - always respects validator-enforced cooldowns first.
        ...
        # Check if cooldowns are globally disabled
        if self.config.get('disable_all_cooldowns', False):
            self.logger.info(f"🚫 Cooldowns disabled - ignoring miner cooldown request for UID {validator.uid} ({reason})")
            return
        
        current_time = time.time()
        
        # Always respect validator-enforced cooldowns first
        if validator.validator_enforced_cooldown_until and validator.validator_enforced_cooldown_until > current_time:
            remaining = validator.validator_enforced_cooldown_until - current_time
            self.logger.info(f"⏳ Validator UID {validator.uid} already has enforced cooldown for {remaining:.1f}s - skipping miner cooldown")
            return
        ...
        if self.config.get('enable_cooldown_logging', True):
            ...
            self.logger.info(f"⏳ Miner cooldown set for UID {validator.uid}: {duration_str} ({reason})")
            self.logger.info(f"   Next available: {time.strftime('%H:%M:%S', time.localtime(validator.miner_cooldown_until))}")
        else:
            self.logger.debug(f"⏳ Miner cooldown set for UID {validator.uid}: {cooldown_seconds}s ({reason})")
    ...
    def _set_emergency_cooldown(self, validator: ValidatorState, cooldown_until: int, reason: str):
        ...
        # Check if cooldowns are globally disabled
        if self.config.get('disable_all_cooldowns', False):
            self.logger.info(f"🚫 Cooldowns disabled - ignoring emergency cooldown request for UID {validator.uid} ({reason})")
            return
        ...
        if cooldown_until > current_time:
            ...
            # Simplified: Check if we can set emergency cooldown (respect validator cooldowns)
            if validator.validator_enforced_cooldown_until and validator.validator_enforced_cooldown_until > current_time:
                self.logger.warning(f"⚠️ Validator UID {validator.uid} has enforced cooldown - skipping emergency miner cooldown")
                return
            
            # Check if miner cooldown is already longer
            if validator.miner_cooldown_until and validator.miner_cooldown_until > emergency_cooldown_until:
                self.logger.warning(f"⚠️ Miner cooldown already set for UID {validator.uid} - not escalating")
                return
            
            validator.miner_cooldown_until = emergency_cooldown_until
            self.logger.warning(f"🚨 DYNAMIC emergency miner cooldown set for UID {validator.uid}: {reason}")
            self.logger.warning(f"   Original cooldown: {cooldown_until}, Emergency miner cooldown: {emergency_cooldown_until}")
    ...
    def _handle_critical_violations(self, validator: ValidatorState, violation_count: int):
        ...
        # Check if cooldowns are globally disabled
        if self.config.get('disable_all_cooldowns', False):
            self.logger.info(f"🚫 Cooldowns disabled - ignoring critical violations for UID {validator.uid} (count: {violation_count})")
            return
        ...
        # Simplified: Set miner cooldown (validator cooldowns are already respected)
        validator.miner_cooldown_until = emergency_cooldown_until
    ...
    def _blacklist_validator_temporarily(self, validator: ValidatorState, violation_count: int):
        ...
        # Simplified: Set blacklist and miner cooldown (validator cooldowns are already respected)
        validator.is_active = False
        validator.emergency_blacklist_until = blacklist_until
        validator.miner_cooldown_until = blacklist_until
    ...
    def _check_and_clear_expired_emergency_blacklists(self):
        ...
        for validator in self.validators.values():
            ...
            # Check if miner cooldown has expired but emergency blacklist is still active
            if (validator.miner_cooldown_until and 
                current_time >= validator.miner_cooldown_until and
                validator.emergency_blacklist_until and
                current_time >= validator.emergency_blacklist_until):
                
                self.logger.info(f"✅ Miner cooldown and emergency blacklist expired for UID {validator.uid}")
                self._safe_reset_validator(validator, "miner cooldown and emergency blacklist")
                cleared_count += 1
    ...
    def _safe_reset_validator(self, validator: ValidatorState, reason: str):
        ...
        # Simplified: Clear emergency restrictions and miner cooldowns (validator cooldowns remain)
        ...
        if hasattr(validator, 'violation_history') and validator.violation_history:
            ...
            if recent_trend == 'increasing':
                ...
                # Simplified: Set a shorter miner cooldown for problematic validators
                extended_monitoring_cooldown = time.time() + 300  # 5 minutes
                validator.miner_cooldown_until = extended_monitoring_cooldown
                self.logger.warning(f"   Extended monitoring miner cooldown set: 5 minutes")
    ...
    def get_cooldown_status(self, validator: ValidatorState) -> str:
        ...
        # FIXED: Check validator-enforced cooldown first
        if validator.validator_enforced_cooldown_until:
            current_time = time.time()
            if current_time >= validator.validator_enforced_cooldown_until:
                status_parts.append("Validator cooldown expired")
            else:
                remaining = int(validator.validator_enforced_cooldown_until - current_time)
                if remaining < 60:
                    status_parts.append(f"Validator cooldown: {remaining}s remaining")
                elif remaining < 3600:
                    status_parts.append(f"Validator cooldown: {remaining//60}m {remaining%60}s remaining")
                else:
                    hours = remaining // 3600
                    minutes = (remaining % 3600) // 60
                    status_parts.append(f"Validator cooldown: {hours}h {minutes}m remaining")
        
        # Check miner cooldown status
        if validator.miner_cooldown_until:
            current_time = time.time()
            if current_time >= validator.miner_cooldown_until:
                status_parts.append("Miner cooldown expired")
            else:
                remaining = int(validator.miner_cooldown_until - current_time)
                if remaining < 60:
                    status_parts.append(f"Miner cooldown: {remaining}s remaining")
                elif remaining < 3600:
                    status_parts.append(f"Miner cooldown: {remaining//60}m {remaining%60}s remaining")
                else:
                    hours = remaining // 3600
                    minutes = (remaining % 3600) // 60
                    status_parts.append(f"Miner cooldown: {hours}h {minutes}m remaining")
        
        # Show "No cooldown" only if neither type is active
        if not validator.validator_enforced_cooldown_until and not validator.miner_cooldown_until:
        ...
        if validator.cooldown_violations > 0:
            status_parts.append(f"Miner violations: {validator.cooldown_violations}")
        
        if validator.validator_reported_violations > 0:
            status_parts.append(f"Validator violations: {validator.validator_reported_violations}")
    ...
    def _check_validators_needing_monitoring(self):
        ...
        for validator in self.validators.values():
            ...
            if (validator.cooldown_violations > violation_threshold and 
                ...
                # Simplified: Apply immediate miner cooldown for critical validators in degraded system
                if system_health_ratio < 0.6 and validator.cooldown_violations > violation_threshold * 1.5:
                    ...
                    validator.miner_cooldown_until = immediate_cooldown
                    self.logger.error(f"🚨 IMMEDIATE miner cooldown applied to UID {validator.uid}: 10 minutes")
        ...
        try:
            ...
            for uid, saved_state in saved_states.items():
                if uid in self.validators:
                    ...
                    # Simplified cooldown tracking restoration
                    ...
                    if validator.validator_enforced_cooldown_until or validator.miner_cooldown_until:
                        cooldown_count += 1
                    ...
                    if (validator.cooldown_violations > 50 or 
                        ...
                        validator.validator_enforced_cooldown_until or
                        validator.miner_cooldown_until):
                        ...
                        if validator.cooldown_violations > 100:
                            remaining_time = ""
                            if validator.emergency_blacklist_until:
                                remaining_seconds = validator.emergency_blacklist_until - current_time
    ...
    #                 # DUPLICATE DETECTION DISABLED - Serve every request from all validators!
    ...
    async def pull_task_from_validator(self, validator: ValidatorState) -> Optional[TaskRecord]:
        ...
        try:
            ...
            # Query the validator
            self.logger.info(f"🔍 DEBUG: About to call dendrite.forward() for validator UID {validator.uid}")
            self.logger.info(f"   Axon info: {neuron.axon_info}")
            self.logger.info(f"   Synapse type: {type(synapse)}")
            self.logger.info(f"   Timeout: {self.config['submission_timeout']}")
            ...
            query_time = time.time() - start_time
            self.logger.info(f"🔍 DEBUG: dendrite.forward() completed in {query_time:.2f}s")
            self.logger.info(f"   Response type: {type(response)}")
            self.logger.info(f"   Response length: {len(response) if response else 'None'}")
            ...
            if response and len(response) > 0:
                ...
                self.logger.info(f"🔍 DEBUG: Full response from validator UID {validator.uid}:")
                self.logger.info(f"   Response type: {type(resp)}")
                # self.logger.info(f"   Response attributes: {dir(resp)}")
                self.logger.info(f"   Response repr: {repr(resp)}")
                ...
                # # Check for task in response
                # if hasattr(resp, 'task'):
                #     if resp.task:
                #         self.logger.info(f"   ✅ Task found: ID={resp.task.id}, prompt={resp.task.prompt[:50]}...")
                #     else:
                #         self.logger.info(f"   ❌ Task is None")
                # else:
                #     self.logger.info(f"   ❌ No 'task' attribute found")
                
                # Check for cooldown info
                if hasattr(resp, 'cooldown_until'):
                    self.logger.info(f"   ⏳ Cooldown until: {resp.cooldown_until}")
                else:
                    self.logger.info(f"   ❌ No 'cooldown_until' attribute found")
                
                # FIXED: Store cooldown info but don't enforce until after task completion
                if hasattr(resp, 'cooldown_until') and resp.cooldown_until:
                    current_time = time.time()
                    if resp.cooldown_until > current_time:
                        remaining_cooldown = resp.cooldown_until - current_time
                        ...
                        # Check if cooldowns are globally disabled
                        if self.config.get('disable_all_cooldowns', False):
                            self.logger.info(f"🚫 Cooldowns disabled - ignoring cooldown from validator UID {validator.uid} ({remaining_cooldown:.1f}s)")
                        else:
                            # Store the cooldown info and track which task it's for
                            validator.validator_enforced_cooldown_until = resp.cooldown_until
                            # Only set pending task ID if we actually have a task
                            if hasattr(resp, 'task') and resp.task:
                                validator.pending_cooldown_task_id = resp.task.id
                                self.logger.info(f"⏳ Validator UID {validator.uid} cooldown info received: {remaining_cooldown:.1f}s - will enforce after task {resp.task.id} completion")
                            else:
                                validator.pending_cooldown_task_id = None
                                self.logger.info(f"⏳ Validator UID {validator.uid} cooldown info received: {remaining_cooldown:.1f}s - no task to track")
                    else:
                        self.logger.info(f"✅ Validator UID {validator.uid} cooldown cleared")
                
                if hasattr(resp, 'cooldown_violations') and resp.cooldown_violations:
                    # FIXED: Track validator-reported violations separately from miner logic
                    old_violations = validator.validator_reported_violations
                    validator.validator_reported_violations = resp.cooldown_violations
                    
                    if resp.cooldown_violations > 0:
                        self.logger.info(f"📊 Validator UID {validator.uid} reported {resp.cooldown_violations} violations")
                        
                        # Don't apply miner penalties for validator-reported violations
                        # The validator is already handling this
        ...
        try:
            ...
            if response and hasattr(response, 'feedback') and response.feedback:
                ...
                if task.submission_success and task.task_fidelity_score is not None:
                    ...
                    # FIXED: Clear pending task and enforce cooldown after successful completion
                    if validator.validator_enforced_cooldown_until and validator.pending_cooldown_task_id == task.task_id:
                        # Check if cooldowns are globally disabled
                        if self.config.get('disable_all_cooldowns', False):
                            self.logger.info(f"🚫 Cooldowns disabled - clearing pending task {task.task_id} without enforcing cooldown")
                            validator.pending_cooldown_task_id = None
                            validator.validator_enforced_cooldown_until = None
                        else:
                            current_time = time.time()
                            ...
                            if validator.validator_enforced_cooldown_until > current_time:
                                remaining_cooldown = validator.validator_enforced_cooldown_until - current_time
                                self.logger.info(f"⏳ Enforcing validator cooldown for UID {validator.uid}: {remaining_cooldown:.1f}s remaining")
                                self.logger.info(f"   Cooldown enforced after successful task {task.task_id} completion and submission")
                            else:
                                # Cooldown expired while we were working on the task
                                validator.validator_enforced_cooldown_until = None
                                self.logger.info(f"✅ Validator cooldown for UID {validator.uid} expired while processing task {task.task_id}")
                else:
                    # Failed submission - increment violations
                    self.increment_cooldown_violations(validator, "Failed submission")
                    
                    # FIXED: Clear pending task and enforce cooldown even on failed submissions
                    if validator.validator_enforced_cooldown_until and validator.pending_cooldown_task_id == task.task_id:
                        # Check if cooldowns are globally disabled
                        if self.config.get('disable_all_cooldowns', False):
                            self.logger.info(f"🚫 Cooldowns disabled - clearing pending task {task.task_id} without enforcing cooldown")
                            ...
                            if validator.validator_enforced_cooldown_until > current_time:
                                remaining_cooldown = validator.validator_enforced_cooldown_until - current_time
                                self.logger.info(f"⏳ Enforcing validator cooldown for UID {validator.uid}: {remaining_cooldown:.1f}s remaining")
                                self.logger.info(f"   Cooldown enforced after failed task {task.task_id} submission")
                            else:
                                validator.validator_enforced_cooldown_until = None
                                self.logger.info(f"✅ Validator cooldown for UID {validator.uid} expired while processing task {task.task_id}")
    ...
    async def process_task(self, task: TaskRecord) -> bool:
        ...
        try:
            ...
            if elapsed_time_since_pull < 14.0:
                wait_duration = 15.0 - elapsed_time_since_pull
    ...
    def print_status(self):
        ...
        validators_on_cooldown = [v for v in active_validators if (v.validator_enforced_cooldown_until and time.time() < v.validator_enforced_cooldown_until) or (v.miner_cooldown_until and time.time() < v.miner_cooldown_until)]
...
async def main():
    ...
    parser.add_argument("--no-cooldown", action="store_true", help="Disable ALL cooldown logic - pull tasks normally ignoring cooldowns")
    ...
    if args.no_cooldown:
        config['disable_all_cooldowns'] = True
        print("🚫 ALL COOLDOWNS DISABLED - miner will pull tasks normally ignoring all cooldown restrictions")
    else:
        config['disable_all_cooldowns'] = False
    ...
    if args.no_cooldown:
        print("🚫 COOLDOWN STATUS: ALL COOLDOWNS DISABLED - miner will ignore all cooldown restrictions")
    else:
