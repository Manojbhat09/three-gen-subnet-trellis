...
class DeepSeekUltraLimitTester:
    ...
    def __init__(self, target_prompt: str, ultra_target: float = 0.96, max_attempts: int = 20):
        ...
        self.used_prompts = set()  # Track used prompts to prevent repetition
    ...
    def query_deepseek(self, message: str, temperature: float = 1.2) -> str:
...
🚨 ANTI-REPETITION ALERT: You MUST create a DIFFERENT prompt!
Used prompts so far: {len(self.used_prompts)} attempts
REQUIREMENT: Generate a COMPLETELY DIFFERENT variation with new descriptors!
...
🔄 VARIATION DESCRIPTORS (rotate to avoid repetition):
Authority: aerospace-grade, military-spec, industrial-grade, defense-grade, space-grade, aviation-standard
Process: ultra-precision, precision-engineered, masterpiece-quality, ultra-detailed, technical-perfection, flawless-execution
Materials: aerospace-alloy, military-grade, industrial-strength, precision-forged, ultra-refined
Finish: museum-quality, gallery-standard, exhibition-grade, award-winning, masterwork-level
...
RESPOND WITH ONLY THE OPTIMIZED PROMPT - NO NO EXPLANATION OR ANYTHING ELSE: """
        ...
        if cleaned_prompt in self.used_prompts:
            print(f"   🔄 REPETITION DETECTED! Forcing variation...")
            # Force variation by increasing temperature
            variation_response = self.query_deepseek(user_message + f"\n\nIMPORTANT: The prompt '{cleaned_prompt}' was already used. Generate a COMPLETELY DIFFERENT variation with different descriptors!", temperature=1.5)
            cleaned_prompt = self.clean_generated_prompt(variation_response)
        
        # Add to used prompts
        self.used_prompts.add(cleaned_prompt)
