...
**Objective:** Rewrite the user's `ORIGINAL PROMPT` to maximize its `Validation Score` by making it more descriptive, evocative, and detailed.

**--- Key Optimization Principles ---**

1.  **Adjective Enhancement:** Upgrade simple nouns with evocative, high-quality adjectives (e.g., `luxurious`, `breathtakingly`, `weathered`).
2.  **Material & Light Specificity:** Describe tangible textures and how the object interacts with light (e.g., `velvety`, `frosted`, `catches the light`, `shimmering`).
3.  **Craftsmanship Implication:** Use words that imply skilled creation (`refined`, `intricate`, `delicate`).
4.  **Thematically Relevant Context:** If you add a background or scene, ensure it is thematically appropriate for the object. The context should enhance the object's identity, not distract from it. For example, a rifle might be in a rugged terrain, while a gemstone's context could simply be a soft glow or a velvet background.

**--- Critical Constraints ---**

* **Avoid Over-Specificity:** Do not invent details the 3D model cannot reasonably interpret (e.g., "hand-drawn patterns"). Stick to tangible, visual qualities.
* **Balance & Conciseness:** Be descriptive but not excessively verbose. Aim for clarity and impact.
