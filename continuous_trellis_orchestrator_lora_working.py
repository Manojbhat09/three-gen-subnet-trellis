...
class ContinuousTrellisOrchestrator:
    ...
    def _check_validator_cooldown_state(self, validator: ValidatorState) -> Dict[str, Any]:
        ...
        if validator.last_task_pull:
            ...
            if time_since_pull < self.config['task_pull_interval']:
                ...
                cooldown_status.update({
                    'available': False,
                    'reason': 'Pull interval not met',
                    'remaining_time': time_until_available,
                    'cooldown_type': 'pull_interval',
                    'recommendation': f'Wait {time_until_available:.1f}s for pull interval'
                })
                return cooldown_status
    ...
    async def process_task(self, task: TaskRecord) -> bool:
        ...
        self.logger.info(f"🔍 Traffic type: {traffic_type.upper()} (expected cooldown: {expected_cooldown}s)")
