...
class ContinuousTrellisSimulator:
    ...
    def _validate_prompt(self, original_prompt: str, optimized_prompt: str = None, ply_data: bytes = None) -> float:
    ...
    async def validate_model(self, task: TaskRecord, ply_data: bytes) -> Optional[float]:
        ...
        try:
            ...
            score = self._validate_prompt(task.prompt, optimized_prompt, ply_data)
    ...
    def print_status(self):
        ...
        # Async operations and blacklisting statistics
...
Provide an optimized version that follows successful patterns from the references."""
    ...
    async def run_simulation(self):
        ...
        try:
            if use_async:
                # ASYNC MODE: Add all tasks to queue for concurrent processing
                for task in tasks_to_process:
                    if not self.running:
                        self.logger.info("🛑 Simulation interrupted.")
                        break
                    await self.task_queue.put(task)
                    self.logger.debug(f"📥 [ASYNC] Queued task {task.task_id}")
                
                # Wait for all tasks to complete
                await self.task_queue.join()
                self.stats['tasks_processed'] = len(tasks_to_process)
            else:
