...
class CompletePromptEnvironmentV2:
    ...
    def _record_high_score_for_meta_learning(self, original: str, successful: str, score: float):
        """PATCH 2: Record high scores for meta-learning (FIXED - less spam)"""
        if score >= self.meta_learn_score_threshold:
            ...
            # Only trigger immediate meta-learning once per episode and for significant improvements
            current_episode = len(self.episode_log)
            if not hasattr(self, '_last_immediate_trigger') or self._last_immediate_trigger != current_episode:
                if len(self.meta_learning_events) >= 2:  # Need at least 2 events to compare
                    latest_score = self.meta_learning_events[-1].score_achieved
                    previous_best = max([e.score_achieved for e in self.meta_learning_events[:-1]], default=0.0)
                    
                    if latest_score > previous_best + 0.03:  # Significant improvement threshold
...
class CompleteRLOptimizerV2:
    ...
    def _generate_final_report(self) -> Dict:
        ...
        if 'metrics' in readiness:
            ...
            print(f"   Reason: {readiness.get('reason', 'Not enough episodes for assessment')}")
