...
class RLLoopAgent:
    ...
    def __init__(self, ollama_url: str = "http://localhost:11434",
        ...
        self.alignment_threshold = 0.3    # Critical alignment threshold - below this causes 0.0 scores
                    ...
                    if attempt.validation_score is not None:
                        if best_attempt is None or attempt.validation_score > best_attempt['validation_score']:
                            best_attempt = {
                                ...
                                'alignment_score': attempt.alignment_score,
    ...
    def optimize_with_rl_loop(self, prompt: str, use_validation: bool = True, prompt_with_context=None) -> Dict[str, Any]:
        ...
        best_prompt = prompt
        best_score = 0.0
        best_alignment_score = 0.0  # Track best alignment score separately
        initial_score = None
        ...
        '''
        {
          ...
          "optimized_prompt": "wbgmsst, an ornate heart-shaped pendant, masterfully crafted with a polished sterling silver frame, intricate gold filigree, and a vibrant central turquoise stone, clean white background",
        ...
        },
        {
          ...
          "strategy_used": "material_focus",
          "exploration_type": "exploit",
          "optimized_prompt": "wbgmsst, an exquisite, ornate heart-shaped pendant, masterfully crafted from gleaming sterling silver with intricate gold filigree that catches the light, featuring a vibrant central turquoise stone that emits a subtle inner glow, set against a soft, luminous white background",
        ...
        if best_prev_attempt:
            ...
            best_alignment_score = best_prev_attempt['alignment_score'] # Update best alignment score
        ...
        for round_num in range(1, self.max_optimization_rounds + 1):
            ...
            if use_validation:
                attempt.validation_score, attempt.alignment_score, attempt.quality_score = self._validate_prompt(prompt, attempt.optimized_prompt)
                ...
                self.logger.info(f"      📊 Quality score: {attempt.quality_score:.4f}")
            else:
            ...
            if attempt.alignment_score and attempt.alignment_score > best_alignment_score:
                best_alignment_score = attempt.alignment_score
                self.logger.info(f"      🎯 New best alignment score: {best_alignment_score:.4f}")
        ...
        result = {
            ...
            'final_alignment_score': best_alignment_score,
            ...
            'alignment_progression': [a.alignment_score for a in attempts],
        ...
        self.logger.info(f"   Best alignment score: {best_alignment_score:.4f}")
    ...
    def _should_converge(self, attempts: List[OptimizationAttempt], current_round: int) -> Tuple[bool, str]:
        ...
        # Check alignment score - critical for avoiding 0.0 validation scores
        if attempts and attempts[-1].alignment_score is not None:
            current_alignment = attempts[-1].alignment_score
            if current_alignment < self.alignment_threshold:
                return False, f"Alignment below critical threshold ({current_alignment:.3f} < {self.alignment_threshold})"
            
            # If we have previous attempts, check if alignment is improving
            if len(attempts) >= 2 and attempts[-2].alignment_score is not None:
                previous_alignment = attempts[-2].alignment_score
                alignment_improvement = current_alignment - previous_alignment
                
                # If alignment is significantly declining, don't converge
                if alignment_improvement < -0.05:  # 5% decline threshold
                    return False, f"Alignment declining significantly ({alignment_improvement:.3f})"
        
        # Original convergence logic for validation scores
        if len(attempts) >= 2:
    ...
    def _make_optimization_attempt(self, prompt: str, strategy: str, exploration_type: str, 
        ...
        strategy_context = self._build_strategy_context(strategy) #- Start with "wbgmsst," and end with ", white background"
...
* **OPTIMIZED (Score: 0.0 ⚠️):** `a weathered small wooden hammer resting on a worn leather workbench amidst tools of various trades in a cozy,
 rustic workshop filled with natural light and the scent of sawdust`
  ...
  "learning_applied": ["insight1", "insight2"]
        ...
        while retries < max_retries:
            try:
                ...
                print(structured_output.get('optimized_prompt', f"wbgmsst, {prompt}"))
    ...
    def _build_previous_attempts_context(self, previous_attempts: List[OptimizationAttempt]) -> str:
        ...
        context = "SCORING EXPLAINATION: \n"
        context += """Alignment Score: Alignment Score (20% weight - CRITICAL THRESHOLD)
What it measures: How well the 3D model matches the text prompt or reference image
For Text-to-3D: Uses CLIP model to compare text prompt vs rendered images
Range: 0.0 to 1.0 (normalized by 0.35 for text prompts)
Critical: Must be ≥ 0.3 or final score = 0.0\n"""
        context += """Quality Score:  Combined Quality Score (75% weight - MOST IMPORTANT)
What it measures: Overall aesthetic and technical quality of rendered images
Components:
Quality Classifier: Binary classification (good/bad quality)
Aesthetic Predictor: Continuous aesthetic score (0-1)
Polynomial Pipeline: Combines both scores using a trained polynomial model
Range: 0.0 to 1.0
Impact: This is the primary driver of your final score\n"""
        context += "Score: score = 0.75 * quality_score + 0.25 * alignment_score \n"
        ...
        for attempt in previous_attempts:
            ...
            alignment_text = f"{attempt.alignment_score:.4f}" if attempt.alignment_score else "pending"
            quality_text = f"{attempt.quality_score:.4f}" if attempt.quality_score else "pending"
            ...
            context += f"  Alignment: {alignment_text} | Quality: {quality_text}\n"
        ...
        if len(previous_attempts) > 1:
            ...
            alignments = [a.alignment_score for a in previous_attempts if a.alignment_score]
            ...
            # Add alignment insights
            if len(alignments) > 1:
                alignment_trend = "improving" if alignments[-1] > alignments[-2] else "declining"
                context += f"\nALIGNMENT TREND: Alignment scores are {alignment_trend}. "
                
                if alignments[-1] < 0.3:
                    context += "⚠️ CRITICAL: Current alignment below 0.3 threshold - this will cause 0.0 validation scores! "
                    context += "Focus on improving alignment by making prompts more specific and relevant."
                elif alignment_trend == "declining":
                    context += "Alignment is declining - focus on making prompts more specific to the original intent."
                else:
                    context += "Alignment is improving - continue with this approach."
    ...
    def _inter_round_learning(self, attempts: List[OptimizationAttempt]):
        ...
        if current.alignment_score and previous.alignment_score:
            if current.alignment_score < previous.alignment_score:
                self.logger.info(f"      ⚠️ Alignment declining: {current.alignment_score:.3f} < {previous.alignment_score:.3f}")
                if current.alignment_score < 0.3:
                    self.logger.info(f"      🚨 CRITICAL: Alignment below 0.3 threshold - will cause 0.0 scores!")
            elif current.alignment_score > previous.alignment_score:
                self.logger.info(f"      ✅ Alignment improving: {current.alignment_score:.3f} > {previous.alignment_score:.3f}")
        ...
        # Also consider alignment for exploration
        if current.alignment_score and current.alignment_score < 0.3:
            self.epsilon = min(0.8, self.epsilon + 0.05)  # Explore more if alignment is poor
            self.logger.info(f"      🔍 Increasing exploration due to poor alignment: {self.epsilon:.2f}")
    ...
    def _validate_prompt(self, original_prompt: str, optimized_prompt: str = None) -> Tuple[float, float, float]:
