...
def smart_load_model(
    ...
    if not os.path.exists(model_path):
        ...
        try:
            ...
            # Download both safetensors and config files
            path = snapshot_download(
                repo_id=original_model_path,
                allow_patterns=[
                    f"{subfolder}/*.safetensors",
                    f"{subfolder}/config.yaml",
                    f"{subfolder}/model_index.json"
                ],
            )
            model_path = os.path.join(path, subfolder)
        except ImportError:
            raise RuntimeError(f"Model path {model_path} not found. Please install huggingface_hub package.")
    ...
    # Always use safetensors if available
    ...
    if not os.path.exists(ckpt_path):
        raise FileNotFoundError(f"Safetensors file not found at {ckpt_path}. Please ensure you have access to the safetensors version of the model.")
