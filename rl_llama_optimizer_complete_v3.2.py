...
class LLaMACustomGenerator:
    ...
    def _build_system_prompt(self, instruction: LLaMAInstruction) -> str:
        base_prompt = f"""You are an expert 3D prompt optimizer. Generate ONLY optimized prompts.

CRITICAL: Your response must ONLY contain the optimized prompt. Do NOT include analysis, reasoning, or explanations.
...
3. Keep the main object from original
4. Add {instruction.focus_area}-focused enhancements
5. Be concise but descriptive

OUTPUT FORMAT: Just the prompt, nothing else.
Example: wbgmsst, precision-engineered steel hexagonal prism, aerospace-grade finish, white background"""
...
Generate ONLY the optimized prompt. Do not explain or analyze.
...
"""
    ...
    def _extract_custom_prompt(self, response: str, original: str) -> str:
        ...
        # First try: Look for CUSTOM_PROMPT section (backward compatibility)
        for line in lines:
            if line.strip().startswith('CUSTOM_PROMPT:'):
                custom_prompt = line.split('CUSTOM_PROMPT:', 1)[1].strip()
                break
        
        # Second try: Look for any line with wbgmsst
        ...
        # Third try: Use first non-empty line as the prompt (new format)
        if not custom_prompt:
            for line in lines:
                line = line.strip()
                if line and not line.startswith(('Strategy:', 'Focus:', 'Optimize:', 'Generate', 'Examples:', 'Your optimized')):
                    custom_prompt = line
                    break
        ...
        if custom_prompt:
            ...
            if not custom_prompt.startswith('wbgmsst'):
                # If it starts with a description, prepend wbgmsst
            ...
            # Final validation - check if it looks like a proper prompt
            if len(custom_prompt) > 20 and 'wbgmsst' in custom_prompt:
                return custom_prompt
        
        # Ultimate fallback
        print(f"   ❌ LLaMA extraction failed, using fallback")
...
class RLLLaMAOptimizerV3:
    ...
    def train_with_checkpoints(self, target_prompts: List[str], 
        ...
        while True:  # Continuous learning loop
            ...
            # Calculate episodes completed for current prompt in current cycle
            ...
            for episode_in_prompt in range(episodes_completed, episodes_per_prompt):
                episode_num = self.training_state.episode + 1  # Continue global episode numbering
                ...
                print(f"   🔄 Global Episodes: {self.training_state.total_episodes_completed}")
                ...
                if user_input == 'q':
                    ...
                    self.training_state.current_prompt_index = 0
                    break  # Break out of episode loop to start new cycle
